...
- No validation command execution by the approval readiness simulator.
- No validation command execution by the execution capability gate simulator.
...
RALPH-034Q implements a planning-only approval readiness simulator:

- `scripts/agent/lib/overnight-approval-readiness-simulator.mjs`
- `scripts/agent/overnight-approval-readiness-simulator.mjs`

The simulator reads only an explicitly supplied hypothetical RALPH-034P human approval checkpoint simulation JSON file. It evaluates source validity, source approval-checkpoint disposition, source authorization/safety claims, checkpoint authority claims, and zero/false safety invariants. For valid readiness-assessment inputs, it identifies whether a future supervised workflow could be considered approval-ready and lists missing approval prerequisites.

Example commands:

```powershell
node scripts/agent/overnight-approval-readiness-simulator.mjs .agent/overnight/human-approval-checkpoint-simulation.json
node scripts/agent/overnight-approval-readiness-simulator.mjs .agent/overnight/human-approval-checkpoint-simulation.json --pretty
```

RALPH-034Q remains **planning-only**. It does not make approval decisions, request approval, grant approval, record approval, create approval evidence, create review evidence, create validation evidence, write runtime state, append task history, append run history, run validation commands, accept review, authorize approval, read real git diff/status, execute workers, execute adapters, execute prompts, execute queued tasks, perform network activity, mutate runtime/evidence state, write reports/run logs, write files, apply changes, stage, commit, or push.

### Execution Capability Gate Simulation

RALPH-034R implements a planning-only execution capability gate simulator:

- `scripts/agent/lib/overnight-execution-capability-gate-simulator.mjs`
- `scripts/agent/overnight-execution-capability-gate-simulator.mjs`

The simulator reads only an explicitly supplied RALPH-034Q approval readiness simulation JSON file. It evaluates source validity, source readiness disposition, source safety/non-authorization invariants, and intended changed-file scope.

Example commands:

```powershell
node scripts/agent/overnight-execution-capability-gate-simulator.mjs .agent/overnight/approval-readiness-simulation.json
node scripts/agent/overnight-execution-capability-gate-simulator.mjs .agent/overnight/approval-readiness-simulation.json --pretty
```

Top-level dispositions:

- `invalid_input` — supplied JSON is not a valid RALPH-034Q approval readiness simulation object or intended changed files cannot be safely classified.
- `blocked_for_execution` — upstream readiness, safety, authorization, forbidden product/runtime/evidence/package/git scope, or other hard blocker prevents future execution capability consideration.
- `requires_higher_capability` — the task may be Markdown/documentation-adjacent but touches high-authority or non-first-capability scope.
- `eligible_for_docs_only_execution` — every intended file is a direct Markdown file under `docs/`, `plans/`, or `reports/`; this still grants no execution authorization.

RALPH-034R remains **planning-only**. It does not grant execution capability, write files, run validation commands, accept review, read real git diff/status, execute workers, execute adapters, execute prompts, execute queued tasks, mutate runtime/evidence state, write reports/run logs, apply changes, stage, commit, or push.
