...
skill: `.claude/skills/queue-run/SKILL.md`.
...
Implement the reusable worker as a checked-in Claude Code skill rather than a one-off prompt
document, following the existing `.claude/skills/cleanup-branches/` convention.
...
long wait (including overnight) is survived, and how much usage the run consumed.
