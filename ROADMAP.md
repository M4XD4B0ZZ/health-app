...
> The former OpenCode CLI worker integration ("Phase C", `scripts/agent/**`) predated the
> Tier system below and was retired along with the rest of the Ralph-Loop / Overnight
> Worker initiative — see the "Ralph-Loop Governance / Overnight Worker (Retired)" section.
...
## Ralph-Loop Governance / Overnight Worker (Retired)

**Status: Retired.** This initiative (task IDs `RALPH-001` through `RALPH-047B`) explored an
agent-neutral, repository-first governance model with the eventual goal of an unattended
"overnight worker" that could pick up and execute queued tasks without supervision.

Over ~35 incremental tasks it built a layered chain of dry-run/simulation tooling (queue
validation, worker-envelope planning, invocation-contract previews, adapter-routing
simulation, review/approval-gate simulation, and finally a single supervised "docs-only"
Markdown-file creator). Every stage was deliberately non-executing by design, and the chain
never produced a working unattended worker: no stage ever invoked a real worker/model, wrote
product code, or committed/pushed on its own. Cline was never installed as an autonomous
worker; the few real Cline sessions run under this track (`CLINE-GOV-*`, `CLINE-REAL-*`,
`CLINE-READ-*`) were manual, supervised, read-only audits or single small diagnostic/test
additions — not unattended execution.

**Decision:** the initiative is retired rather than continued. `RALPH-034V` (the last
in-progress task) is cancelled, not completed. The repository does not currently contain a
usable autonomous overnight worker.

**What was removed in `RALPH-RETIRE-001`:** the runtime/simulator scaffolding
(`scripts/agent/**`, `.agent/**`, `tasks/`, `runs/`, `validation/`, `review/`), the associated
plans (`plans/RALPH_*`, `plans/AUTONOMOUS_WORKER_LOOP_IMPLEMENTATION_PLAN.md`), historical
reports (`reports/RALPH-*`, `reports/CLINE-*`, `reports/morning-review.md`), and transition
docs (`docs/RALPH_LOOP_TRANSITION_NOTES.md`, `docs/CLINE_RALPH_WORKER_SETUP.md`,
`docs/CLINE_FIRST_DRY_RUN_CHECKLIST.md`). See
`reports/RALPH_RETIRE_001_DEAD_RUNTIME_CLEANUP.md` and Git history for the full inventory and
reasoning — none of it is reproduced here.

**Reserved task-ID range:** `RALPH-001` … `RALPH-047B` and `CLINE-GOV-001` … `CLINE-GOV-009`,
`CLINE-REAL-002` … `CLINE-REAL-012`, `CLINE-READ-001` are retired and must never be reused for
new tasks.
...
Status: `done` — RESOLVER-V3-015A closed the dedicated private schema/RLS persistence blocker
...
**Persistence blocker (closed):** Existing `food_resolver_runs` columns/legacy metadata and `food_query_cache_results` could not safely represent the full typed V1 private observation contract; RESOLVER-V3-015A now provides the dedicated private boundary.
...
**Acceptance:** One run yields auditable non-authoritative evidence without a second AI call; durable acceptance is satisfied by RESOLVER-V3-015A.
...
**Implementation:** `20260720120000_create_resolver_observations.sql` provides private owner-RLS JSONB V1 storage and the write-only Supabase adapter; focused migration, writer, resolver-observation integration, and logger tests plus `npm run verify` passed. The persistence blocker is closed; RESOLVER-V3-016 remains `todo` and may now begin.
...
the audit table is genuinely append-only. Both blockers are cleared, and this task builds on both.
**Technical inventory (pre-implementation):** `personal_resolution_memories.scope_key` is built from
the **resolved food identity** (`${foodCatalogRef.source}:${foodCatalogRef.sourceId}`), never from raw
or normalized query text (confirmed by reading RESOLVER-V3-026's `resolvePreparedNutritionInputs.ts`
call site) — so a read path can only answer "has this owner confirmed this exact, already
independently-found catalog item before?", never "what did this raw text mean before?". No AI source is
wired into any production resolver (`SequentialFoodCatalogResolver`'s composed `resolverSources` in
`container.ts` are `user`/`bls`/`off`/`usda` only — `ai` appears solely in the source-priority ordering
constant, never in an actual source instance), confirming RESOLVER-V3-010 (hybrid production wiring)
remains genuinely blocked and unrelated to this task. `personal_resolution_memories` already grants
`authenticated` `select` under the RESOLVER-V3-017 owner-scoped `for all` RLS policy, so a production
read adapter needs zero new migration. `LogFoodFromRawInputUseCase` only checks `resolved.score >= 0.7`
(not `decision.status`) to accept a resolver candidate, while `LogMealFromRawInputUseCase` checks
`status === 'accepted'` — both were confirmed by reading their source before designing the override.
**Implementation (2026-07-21):** Added the closed `personal-resolution-memory-read-v1` contract
(exact-match `{sourceType, sourceId}` targets only — identical scope-key shape to the write path),
`PersonalResolutionMemoryReadEligibilityPolicy` (the single place level→eligibility is decided: only
`P2_confirmed` is `deterministicReuse`-eligible, `P1_provisional` is `preferred`-only and must remain
correctable, `P0_observed` is neither — Decision Record §6 verbatim), the owner-scoped
`PersonalResolutionMemoryReadRepository` port (`findActiveByScopeKeys`, `active`-status rows only, fails
closed to an empty array), `ReadPersonalResolutionMemoryUseCase` (validates contract/owner/targets,
fails closed on any repository throw), and `SupabasePersonalResolutionMemoryReadRepository` (a single
read-only `select` on `personal_resolution_memories`, no new grant/migration needed). Resolver
integration is a new decorator, `PersonalResolutionMemoryAwareFoodCatalogResolver`, wrapping any
`FoodCatalogResolver` — deliberately **not** a change inside `SequentialFoodCatalogResolver` itself, to
keep this new capability fully isolated from that file's existing, heavily-tested internals. It excludes
`'user'`-sourced (alias) candidates, fails open on any missing owner/lookup error/no-match (returning the
exact original decision object, proven by reference-identity assertions), and on a `P2_confirmed` match
deterministically selects that candidate as `best` with `status: 'accepted'` and a fixed `0.95` score
(clearing both `LogFoodFromRawInputUseCase`'s and `LogMealFromRawInputUseCase`'s acceptance gates) plus a
`PERSONAL_MEMORY_P2_CONFIRMED_AVOIDED_AI` reason code — or, for `P1_provisional` only, appends a
`PERSONAL_MEMORY_P1_PREFERRED` reason code without overriding `best`/`status` (correctable, per the
Decision Record). `PersonalResolutionMemoryReadTelemetry` records real counts
(`targetCount`/`matchCount`/`deterministicReuseMatchCount`/`preferredMatchCount`) plus an honest
`avoided` boolean (only `true` when the override actually changed the outcome, not on a redundant
confirmation of an already-accepted decision) on every lookup — never hard-coded, unlike the metrics gap
the RESOLVER-V3-020/022 post-implementation review found. The production telemetry adapter,
`ConsolePersonalResolutionMemoryReadTelemetry`, is dependency-free (counts/enums only, no raw input, no
food identity, no owner ID) and gated behind the existing `isDebugLoggingEnabled()` flag. Wired in
`container.ts` as one composition-root change: `foodCatalogResolver` is now the decorator wrapping the
previously-direct `SequentialFoodCatalogResolver` instance; both `LogFoodFromRawInputUseCase` and
`LogMealFromRawInputUseCase` already depended on the `FoodCatalogResolver` interface, not the concrete
class, so no other call site changed.
**Migration:** none. `personal_resolution_memories` already grants `authenticated` `select` under the
RESOLVER-V3-017 migration's owner-scoped `for all` RLS policy; the new adapter filters `owner_id` in the
query and RLS enforces the same scope again independently (defense in depth).
**Known/residual limits (explicit, not claimed complete):** this decorator's added reason codes are not
persisted into the pre-existing `ResolverRunLogger`/`ResolverObservationWriter` telemetry sinks, because
those already fire _inside_ `SequentialFoodCatalogResolver.resolve()`, before this decorator sees the
decision — they still record the base (pre-override) decision, which is itself correct (it reflects what
the deterministic sources found) but means this task's own telemetry is a separate, additional channel,
not a retrofit of the existing one; a future task may unify them. No near-match/fuzzy transfer exists
(exact scope-key match only, by design). No AI/hybrid production wiring was added or touched — RESOLVER-
V3-010 remains blocked and unrelated.
**Contract doc:** [`docs/domains/ZERA_PERSONAL_RESOLUTION_MEMORY_READ_CONTRACT_1.md`](../docs/domains/ZERA_PERSONAL_RESOLUTION_MEMORY_READ_CONTRACT_1.md).
**Verification:** `npm run verify` (typecheck + lint + format:check + full test suite) passed — 182 test
suites, 1601 tests, all green, no regressions (up from 178 suites/1571 tests before this task). New/focused
suites: `PersonalResolutionMemoryReadEligibilityPolicy.test.ts` (level→eligibility mapping for all three
levels); `ReadPersonalResolutionMemoryUseCase.test.ts` (match mapping per level, target dedup, defense-in-
depth against an unrequested scope key, closed invalid-request/no-owner error codes, repository-throw
surfaced as `failed` not an exception); `SupabasePersonalResolutionMemoryReadRepository.test.ts` (exact
select/filter shape, fail-closed on query error and on missing owner/scope keys, without ever querying);
`PersonalResolutionMemoryAwareFoodCatalogResolver.test.ts` (fail-open on no owner/no candidates/no match/
any throw — proven via reference-identity, not just value equality; `'user'`-source exclusion;
deterministic override from both `ambiguous` and `rejected` base status; no redundant override/avoided-
flag when already accepted as the same candidate; P1 annotate-only behavior; P0 fully ignored; query
...
Approval Gate) is now `done` — the real `ResolverKnowledgeReviewService` no longer approves a
candidate while coherent contradiction evidence is present (see its own ROADMAP entry and
`reports/RESOLVER_V3_037_CONTRADICTION_APPROVAL_GATE_REMEDIATION_REPORT.md`). `RESOLVER-V3-037` is
added as an explicit dependency so this task's eventual re-decision can rely on both the original
RESOLVER-V3-023 benchmark evidence and this focused remediation evidence together. This task itself
remains `todo` and was **not** started by this change.
...
Status: `done — executable offline candidate and evidence infrastructure complete; live superiority unverified` (2026-07-27)
...
**Phase B3 Pre-PR Remediation 2 (2026-07-31, same branch, before any PR was opened):** a second
independent review found five further defects, all fixed with zero provider calls: (1) success
accounting counted usage records rather than real HTTP requests — `summarizeSuccessUsage` now sums
the real, measured `counts.providerHttpRequests` per ledger entry as its own exact dimension,
separate from confirmed tokens/cost, and a structurally successful run with any incomplete-usage
call is now `'partial'` (a safe reserved-cost upper bound, never a fabricated USD 0.00); (2) the
budget gate's reservation count was reachable under provider-call-sounding names — a new, read-only,
transport-authoritative cumulative counter
(`ProtocolV4HumanLiveExecutionContext.getCumulativeProviderHttpRequestCount()`, incremented at the
private counting-transport boundary immediately before every real `fetch`) now supplies
`providerHttpRequests`, while the gate's own count is named only `aiDispatchReservations`; (3) the
Development Runner's catch block previously marked the lease `terminal_failure` BEFORE attaching the
usage snapshot, and a lease-finalization failure could lose both — reordered to attach the snapshot
to the original error first, attempt lease finalization in its own nested `try`/`catch`, and never
replace the original error; a new secret-free `leaseFinalization` status
(`'terminal_failure_confirmed'` | `'failed_to_persist'`) is always attached; baseline computation and
gate construction moved inside the `try` so a baseline failure after `executing` no longer leaves
the lease stuck; (4) an absent failure snapshot no longer silently means exact zero — only a
verified pre-dispatch error class does; anything else reports `accounting: 'unknown'` with `null`
fields, never a fabricated `0`; (5) `LauncherError` no longer trusts its whole message — every
constructor now takes `(code, internalDetail)`, `code` must be one of a fixed
`KNOWN_LAUNCHER_ERROR_CODES` set (the only thing ever surfaced), and `internalDetail` (paths, foreign
error messages, compiler output, submitted values) never reaches stdout/stderr/the summary. The
...
**Final Phase-A Execution Closure remediation (2026-07-28, basis
`e1d3801b6f8e645910534f95afde868212c3853f`, PR #192's merge commit into `chore/clean-arch-structure`;
PR #192 itself is kept merged, not reverted):** an independent post-merge review of PR #192 found the
"Final Evidence-Lineage" work above still had five categories of residual defects: (1) the all-path
`runProtocolV4Attempt` wrapper still left a call stuck at `dispatched` with no terminal record on a
rejected `attempt()` promise or a post-dispatch exception (metadata extraction/terminal-builder/
validator throwing); (2) `assertProviderRunIdentityMatchesAttemptContext` returned success on a fully
absent or partial provider run identity, and `meta.failureKind ?? null` silently turned an unset
`failureKind` into "success"; (3) `assertDevelopmentAuthorized`/`assertHoldoutAuthorized` still
accepted caller-supplied `artifactTargetUnused: boolean` and `remainingCalls`/etc. fields tautologically
derived from the authorization's own `max*` fields, and the atomic consumption marker was never read
back before reuse; (4) the evaluation adapter used a benchmark-local structural approximation instead
of the real, pinned `RepresentativeHybridV1LiveMetrics.ts`/`RepresentativeHybridV1LiveReportBuilder.ts`
G2 evaluator, and the Development Runner dispatched every observation against the same fixed input
(`"Testlebensmittel"`) with `criticalError` hand-set to `false`; (5) `runProtocolV4MiniProtocolRun()`
never executed a single Holdout observation, stopping immediately after the Holdout gate.

All five are closed on branch `claude/resolver-v3-048-final-execution-closure`: `runProtocolV4Attempt`
now wraps its dispatched body in try/catch, closing any still-`dispatched` call with a new, closed
`internal_wrapper_error` terminal (via `closeDispatchedCallWithInternalError`, itself a safe no-op if a
race already completed the call) before re-throwing, so every dispatch ends terminal exactly once, no
exception path excepted; `buildProtocolV4TerminalFromProviderMetadata` now requires a complete provider
run identity (all seven `REQUIRED_RUN_IDENTITY_FIELDS`) and an explicitly-set `failureKind`, throwing
`PROTOCOL_V4_PROVIDER_RUN_IDENTITY_MISSING`/`_FIELD_MISSING`/`PROTOCOL_V4_PROVIDER_FAILURE_KIND_MISSING`
rather than defaulting; `assertDevelopmentAuthorized`/`assertHoldoutAuthorized` now take
`artifactStoreRoot`/`artifactRelativePath` (queried live against `isProtocolV4ArtifactTargetUnused`/
`isProtocolV4AuthorizationConsumedAtomically`) and independent `consumedBudget`/`plannedBudget` figures
(the Development/Holdout Runners' own `LiveProviderBudgetGate.snapshot()`, never derived from the
authorization's own ceiling), checked as `consumed + planned > max`; the Artifact Store's
`writeProtocolV4ArtifactExclusive` now commits via `fs.linkSync` (atomic, fails `EEXIST`) instead of a
`existsSync`-then-`renameSync` race, and validates the fully-resolved final path (root + relativePath),
closing a `../..`-in-`relativePath` traversal gap the old root-only check missed; a new
`ResolverV3048ProtocolV4RealEvaluator.ts` wires in the REAL, unmodified evaluator end to end — real,
zero-network Variant A (BLS-only) and the real `NoopVariantBProvider` computed once as a shared,
candidate-independent baseline, each candidate's own executed `VariantCRawCaseResult`s judged against
real corpus ground truth via `evaluateVariantCCase` (unmodified), and `buildRepresentativeHybridV1LiveReport`
(unmodified) producing the actual G2-A..G2-G gate verdicts — while the Development Runner now looks up
each observation's real frozen `BenchmarkCase` (`protocolV4ScenarioByScenarioId()`) and derives
`criticalError`/`identificationOutcome`/clarification/abstention from the real judged result, never
hand-set; the zero-network dry-run fixtures (`ResolverV3048ProtocolV4Fixtures.ts`) now echo each
observation's own real `expectedComponents[0]` (name/source ID) back through the fake AI-interpretation
envelope and fake catalog source, in the correct per-candidate S0/S1 wire schema
(`resolvedInterpretedEnvelopeForSchema`) — the honest, documented zero-network stand-in for "a
competent, correct interpretation/source match" (the one thing Phase-A cannot itself produce without a
live provider), closing a latent S0/S1 envelope-schema mismatch that had silently made every H1/H2
AI-routed dispatch fail identification regardless of input; a new
`ResolverV3048ProtocolV4HoldoutRunner.ts` (reusing the Development Runner's generalized
`runOneObservation` with an explicit Holdout execution-tree hash/evidence root/call-ID namespace) now
actually dispatches every planned Holdout observation for the selected candidate and seals its full
checkpoint/raw-results/category-table/telemetry/ledger/evaluation artifact set;
`runProtocolV4MiniProtocolRun()` now executes that Holdout Runner after the Holdout gate, persists and
reads back all six Holdout artifacts through the same atomic Artifact Store, and only then stops (no
further phase remains in the protocol); and `validateCandidateSelectionRecord` gained a strengthened
sibling, `validateCandidateSelectionRecordAgainstEvidence`, which re-derives
`developmentEvidenceRootHash`/`developmentArtifactHashes` from the real `ProtocolV4DevelopmentEvidence`
and requires each stored `evaluations[id]` to be canonically identical to that candidate's own
validated artifact content — rejecting a re-hashed, internally self-consistent Selection Record whose
evidence-root/artifact-hash claims were swapped for different evidence, even with the winner unchanged
(the base validator alone cannot see this, since it only checks a record's own internal
self-consistency).

Wiring in the real evaluator surfaced two genuine, honest architectural findings that required
adjusting `selectCandidate`'s Development-time eligibility screen (`isProtocolV4CandidateEligibleAtDevelopmentTime`,
documented in code at both call sites): the real evaluator's joint gate combinator
(`overallGateVerdict` in `RepresentativeHybridV1LiveReportBuilder.ts`) structurally resolves every
mandatory gate to `not_evaluable` whenever Holdout data is absent — which it always is at
Development-Selection time, since Holdout only ever runs afterwards for the single already-selected
candidate — so requiring the literal `allMandatoryG2CriteriaPass` boolean here (as the pre-existing
code did) made every candidate permanently ineligible once the real evaluator replaced the old
...
tie-breaker the rule already documented but the code had never actually implemented. The real, binding
G2 pass/fail decision remains exclusively a post-Holdout, joint-partition determination, never claimed
here. 96 tests pass (`ResolverV3048ProtocolV4*.test.ts`, including 4 new regression tests for path
traversal, concurrent-writer artifact-store races, Selection-Record-vs-evidence binding, and
per-observation Holdout input diversity); `npm run typecheck`/`lint`/`format:check` are clean. Provider
calls remain exactly **0**; provider cost remains exactly **USD 0**; no credential was read; the seven
V3-039 evidence files, corpus, ground truth, and the corrected G2 evaluator's own logic are unchanged.
The proposal-only budget (352 calls / USD 5.586944) is unchanged in value and remains explicitly **not
...
Status: `done` (implemented, tested, `npm run verify` green; see "Implementation notes" below for
gate details and focused remediation evidence)
...
**Implementation notes (RESOLVER-V3-037, 2026-07-22):** Added a tenth closed result,
`blocked_contradiction`, to `ResolverKnowledgeReviewResult`
(`src/features/nutrition/domain/models/ResolverKnowledgeReview.ts`) — distinct from
`blocked_privacy`/`validation_failed`/`invalid_transition`, none of which is reused. Within
`ResolverKnowledgeReviewService.review()`'s `approve` branch
(`src/features/nutrition/application/knowledge/ResolverKnowledgeReviewService.ts`), a new gate runs
**before** the pre-existing independent-user-evidence check: it validates
`contradictionStatus`/`contradictingEvidenceCount` runtime coherence (defensively, since the service
accepts whatever its candidate-reader port returns, not only what the static type promises) and
returns `blocked_contradiction` for the coherent `present`/`>0` state, or `validation_failed` for any
incoherent/malformed combination (`none`+nonzero, `present`+`0`, unknown status, negative/fractional/
`NaN`/`±Infinity`/non-number count). No numeric threshold exists — a count of `1` and a count far
larger than `1` are refused identically, for all five candidate types, regardless of risk or
supporting-evidence count. A blocked attempt returns before any persistence:
`ResolverKnowledgeReviewRepository.applyDecision()` is never called, so candidate status, lifecycle
events, the approved-payload store, and the review-event log are all provably unchanged (verified via
a repository spy plus full before/after store snapshots). A repeated blocked attempt with the same
`decisionId` is re-evaluated from scratch and returns `blocked_contradiction` again — never
`already_applied`, since nothing was ever persisted for it. The review contract version remains
`resolver-knowledge-review-v1` (a fail-closed tightening of the existing gate, per the RESOLVER-V3-028
precedent); no migration was added (the review-event `result` column has no enumerated database check
constraint, confirmed directly from the migration file); no Supabase adapter, DI/container change, or
production caller was added (this service remains unwired — zero production callers exist). See
`docs/domains/ZERA_RESOLVER_KNOWLEDGE_REVIEW_CONTRACT_1.md` §"Amendment (RESOLVER-V3-037)" for the
full gate specification.

**Tests:** `src/features/nutrition/__tests__/ResolverKnowledgeReview.test.ts` — 34 pre-existing tests
unchanged and green, plus 29 new tests (exact `LBV2-GC-DEV-006` fixture-state reproduction with a
zero-mutation/no-`applyDecision`-call proof; contradiction checked before independent-user evidence
in both directions; zero-threshold proof (count `1` and `1,000,000` block identically; a large
supporting-evidence count cannot override); all seven runtime-coherence cases
(`none`+`0` valid, `present`+positive valid-but-blocked, `none`+nonzero/`present`+`0`/negative/
fractional/`NaN`/`±Infinity`/non-number/unknown-status all `validation_failed`, each with zero
mutation); all five candidate types blocked with no exemption; `needs_more_evidence`/`reject` with
`CONTRADICTING_EVIDENCE` remain legal; idempotent re-evaluation of a repeated blocked attempt) — 63
tests total, all green. Two RESOLVER-V3-023 benchmark tests that encoded the discovered defect as
current behavior were updated to assert the real, now-fixed service's behavior (not the frozen
fixture data, which is untouched):
`src/features/nutrition/benchmark/learningV2/__tests__/LearningBenchmarkV2GlobalCandidateAdapter.test.ts`'s
`contradictionGateApprove` step now asserts `blocked_contradiction` (previously asserted the defect's
`applied`), and its `rollback`/`rollbackRetry` steps now assert `invalid_transition` (previously
`applied`/`already_applied`) — a documented historical-fixture consequence, since `LBV2-GC-DEV-006`
is the corpus's only `review-rollback`-tagged scenario and its candidate is now correctly never
approved (rollback/revoke coverage for a legitimately approved, contradiction-free candidate remains
green in the dedicated review-service suite above). `runLearningBenchmarkV2.test.ts` gained one new
test documenting that INV-07 now reports `passed` while INV-10/INV-11 newly report `failed` against
this same fixture for the reason above — `systemVerdict` remains `NOT_PASSED`. One narrow,
pre-existing reporting bug this consequence exposed was also fixed:
`buildLearningBenchmarkV2Reports.ts`'s "Discovered defects" narrative previously special-cased only an
`INV-07` failure and otherwise unconditionally claimed no invariants failed, which would now be false;
it was corrected to a three-way branch (no failures / `INV-07` failure / any other failure) — this is
the only benchmark-harness (non-test, non-docs) file touched by this task, and the frozen corpus
fixture, registry, hash, and canonical V3-023 reports are otherwise untouched.

**Verification:** `npm run verify` (typecheck + lint + format:check + test) — green, 0 type errors,
0 lint errors, 0 format violations. `git diff --name-only` against `origin/chore/clean-arch-structure`
touches only: `src/features/nutrition/domain/models/ResolverKnowledgeReview.ts`,
`src/features/nutrition/application/knowledge/ResolverKnowledgeReviewService.ts`,
`src/features/nutrition/__tests__/ResolverKnowledgeReview.test.ts`,
`src/features/nutrition/benchmark/learningV2/__tests__/LearningBenchmarkV2GlobalCandidateAdapter.test.ts`,
`src/features/nutrition/benchmark/learningV2/__tests__/runLearningBenchmarkV2.test.ts`,
`src/features/nutrition/benchmark/learningV2/buildLearningBenchmarkV2Reports.ts`,
`docs/domains/ZERA_RESOLVER_KNOWLEDGE_REVIEW_CONTRACT_1.md`, the new
`reports/RESOLVER_V3_037_CONTRADICTION_APPROVAL_GATE_REMEDIATION_REPORT.md`, and this
`ROADMAP.md`/`handoffs/latest-handoff.md` entry — zero changes to `supabase/migrations/**`,
`supabase/functions/**`, `package.json`, `package-lock.json`, any environment file,
`src/infrastructure/di/container.ts`, any journal/UI file, the Learning Benchmark V2 corpus/registry
files, or the canonical `reports/RESOLVER_V3_LEARNING_BENCHMARK_V2_REPORT.md`/
`reports/resolver-v3-learning-v2-benchmark.json` (verified directly; the latter two remain historical
evidence of the pre-fix state, unmodified).

**Focused `INV-07` remediation verdict:** `PASSED` (see the remediation report). This does **not**
mean the original V3-023 Learning Benchmark V2 report is retroactively `PASSED`, that the full
benchmark was rerun, or that any production resolver effect now exists.
