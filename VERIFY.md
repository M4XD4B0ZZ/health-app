...
**Handoff schema boundary:** Verification disclosure inside handoffs is mandatory (checks run, rationale, result), but `VERIFY.md` does not own the full handoff schema. The normative full schema owner is `AGENTS.md` (see its "Handoff Requirements" section).
