...
const testInputs = ['ei', 'zwei eier', '200g quark', 'buttertoast', 'zwei scheiben schinken'];
...
for (const input of testInputs) {
  ...
  try {
    ...
    const estimatedMacros = estimateMacros(
      canonicalEntity?.id || firstItem.name,
      finalQuantityGrams,
    );
...
for (const result of results) {
  if (result.error) {
    ...
    console.log(
      `| ${result.input} | ${status} | ${result.finalQuantityGrams}g | ${result.estimatedMacros.kcal} | ${result.canonicalEntity || 'none'} | ${persist} |`,
    );
  }
}
