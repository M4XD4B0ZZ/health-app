...
5. **Verification executed:** `npm run typecheck`, `npm run lint`, `npm run format:check` (repo-wide, all clean); `npx jest --runInBand src/features/nutrition/benchmark/protocolV4` (4 suites / 96 tests, all passing); `git status`/`git diff --check`; manual `git diff --stat` review confirming only `protocolV4/` implementation/test files changed, no frozen/protected path touched.
