...
- **Auth is already partially scaffolded, but not implemented, wired to UI, or usable** —
  ...
  - **Implication for this plan**: ACC-001 must not re-propose "add an auth port/use case" as
    a new task — that scaffold exists. The phased roadmap in §17 sequences _around_ P2-008
...
- `user_entitlements` (`supabase/migrations/20260712_add_user_entitlements_table.sql`):
  **read-only** for the owning user (`SELECT` policy only); all writes happen via a
  ...
  exists on purpose. This is the right template for any future _server-computed_ field.
...
This matrix answers deliverable #3. The live open question is not _whether_ accounts are
optional (settled) but _which capability ships first_ once an account exists — see §12.
...
| `foodCatalogRef` (pointer to a Food Catalog row: source/sourceId/displayName/confidence)       | **A** (as a reference field only)                                         | The _reference_ syncs with the entry; the catalog row it points to is **C** and must never be re-uploaded or duplicated per user.                                                                              |
...
| Provenance/source metadata on catalog matches                                                  | **A** (as part of the entry's `foodCatalogRef`/`resolverDecisionSummary`) | Needed to explain _this user's_ logged entry; the catalog row itself remains **C**.                                                                                                                            |
...
**Preserved principle:** every user-created record keeps the _same_ stable ID across local
...
| C. Revision-based record sync                                                                           | Per-record monotonic revision counter, incremental pull by revision.                                                                | Good primitive, but insufficient alone — needs an explicit outbox for the write path and explicit tombstones for deletion; C is a _component_ of the recommended answer, not a complete architecture by itself.                         |
...
| F. Hybrid (local durable write → outbox → background push → incremental pull → optional Realtime hints) | D, plus Realtime used only as an optional low-latency _hint_ to trigger an incremental pull sooner — never as the data path itself. | **Recommended.**                                                                                                                                                                                                                        |
...
and it should stick" is the safer default; the Correction Log needs _no_ conflict rule at all
...
- **MVP decision (recommended, pending final approval per §24):** **(C) Backup first, sync
  ...
  only thing Phase 3 adds is _turning on_ background incremental pull + the conflict rules
...
3. **Local data + populated cloud** (device had been used locally before ever logging in,
   _and_ the account already has data from another device). This is the only genuinely
...
6. **Login to a different account after previous sync**: must not silently mix the two
   ...
   (records that carry the _previous_ account's `user_id`) before starting the new account's
...
4. **Progress state**: visible, non-blocking indicator while the incremental pull (from
   revision 0) runs; the app remains usable for _new_ local logging concurrently — a restore
...
- **Server-controlled fields cannot be arbitrarily reassigned by clients**: `revision` and
  ...
  submit the _content_ change; the server is the sole authority for revision/ownership
...
- **Auditability without exposing private health data**: `sync_mutations` (idempotency
  ledger) gives an audit trail of _what was attempted and when_ without needing to expose
...
is a per-record tombstone that other devices _learn about_ over time via the normal pull
...
| 14  | Wrong-account login (e.g. typo'd into someone else's test account) | Standard first-login flow (§13) applies exactly as for any other account — RLS guarantees no cross-account data leakage regardless                                                                   | None to the _other_ account's data (RLS-enforced); the user's own local data follows §13's normal state-machine branches                                                                                                 | N/A                                                                                                    | User realizes and logs out/into the correct account | N/A                                                    |
| 15  | Logout with unsynced changes                                       | §10: offered explicit "keep on this device" vs "remove from this device"; unsynced changes are not silently discarded by the logout action itself                                                    | Only if the user explicitly chooses "remove from this device" while changes are still unsynced — the UI must surface „Änderungen ausstehend" _before_ offering that choice so the user isn't discarding data unknowingly | N/A                                                                                                    | Yes (explicit choice)                               | N/A                                                    |
...
| 20  | Clock skew (device clock wrong)                                    | No correctness impact on conflict resolution, because §9/§10 use server-assigned revision, not client timestamp, as the tie-breaker; only _display_ of "last changed X ago" could look odd           | None                                                                                                                                                                                                                     | N/A                                                                                                    | No                                                  | N/A                                                    |
...
- **Do all durable records have stable IDs?** Yes, today, for every domain in §6 classified
  **A**. The ID _format_ (§7) is recommended to change for new records only, not backfilled.
...
- **ACC-002 — Saved Meal soft-delete.** Add `deletedAt`/tombstone semantics to
  ...
  pattern. _Expected files:_ `src/features/nutrition/domain/models/SavedMealTypes.ts`,
...
- **ACC-003 — UUID/ULID record identity for new records.** Introduce a standards-based ID
  ...
  record IDs are left untouched (additive, no backfill/rewrite). _Expected files:_
...
- **ACC-004 — Local sync-readiness fields (schema-only, no sync engine yet).** Add optional
  ...
  migration on top of Phase 0. _Expected files:_ domain model files for
...
- **ACC-005 — OAuth native wiring (app.json + dependencies).** Registers Apple/Google OAuth
  ...
  scheme to `app.json`, adds `expo-web-browser`/`expo-apple-authentication`. _Expected
  files:_ `app.json`, `package.json`/`package-lock.json` (both protected — explicit approval
...
- **ACC-006 — Settings login/logout screen.** Presentation-layer screen using the existing
  ...
  `expo-web-browser`, handles the redirect. _Expected files:_ a new
...
- **ACC-007 — Session-storage hardening decision.** Evaluate and, if approved, implement
  ...
  persistence. _Expected files:_ `src/infrastructure/supabase/supabaseClient.ts`,
...
- **ACC-008 — Local outbox + idempotency key infrastructure.** New local-only durable outbox
  ...
  _Expected files:_ a new port (e.g. `SyncOutboxRepository`) + `AsyncStorage`-backed
...
- **ACC-009 — Server schema: `journal_entries`, `journal_corrections`, `saved_meals`,
  ...
  with RLS per §16.** _Expected files:_ new `supabase/migrations/*.sql` (protected — explicit
...
- **ACC-010 — Push path (outbox → authenticated upload → ack).** Wires ACC-008's outbox to
  ACC-009's schema via authenticated Supabase calls. _Expected files:_ new infrastructure
...
- **ACC-011 — First-login adoption flow (§13's state machine) + restore flow (§14).**
  _Expected files:_ new use case(s) under `src/features/sync/application/`, presentation
  wiring in the account screen from ACC-006, `docs/MANUAL_TESTING_GAPS.md` entry. Depends on:
  ACC-010.
- **ACC-012 — Sync-status UX (§20 states).** _Expected files:_ small presentation-layer
