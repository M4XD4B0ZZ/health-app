...
describe('canonical-queue-entry-writer-probe', () => {
  ...
  it('explicit execute creates exactly the fixed artifact in an isolated project root', async () => {
    const projectRoot = tempRoot();
    const result = await runCanonicalQueueEntryWriterProbe({ projectRoot, execute: true, preWriteEvidence: cleanEvidence(), postWriteEvidence: targetCreatedEvidence(), expectedChangedFiles: [TARGET_PATH] });
  ...
  it('refuses overwrite via create-only semantics', async () => {
    const projectRoot = tempRoot();
    const first = await runCanonicalQueueEntryWriterProbe({ projectRoot, execute: true, preWriteEvidence: cleanEvidence(), postWriteEvidence: targetCreatedEvidence(), expectedChangedFiles: [TARGET_PATH] });
    const second = await runCanonicalQueueEntryWriterProbe({ projectRoot, execute: true, preWriteEvidence: cleanEvidence(), postWriteEvidence: targetCreatedEvidence(), expectedChangedFiles: [TARGET_PATH] });
  ...
  it('refuses arbitrary, absolute, drive-qualified, traversal, and protected target paths', () => {
    ...
    assert.ok(validateFixedTarget('runs/current-run.json').findings.some((entry) => entry.code === 'protected_target_refused'));
    assert.ok(validateFixedTarget('validation/validation-results.jsonl').findings.some((entry) => entry.code === 'protected_target_refused'));
    assert.ok(validateFixedTarget('review/review-results.jsonl').findings.some((entry) => entry.code === 'protected_target_refused'));
    assert.ok(validateFixedTarget('handoffs/latest-handoff.md').findings.some((entry) => entry.code === 'protected_target_refused'));
    assert.ok(validateFixedTarget('.agent/runtime/sandbox/x.json').findings.some((entry) => entry.code === 'protected_target_refused'));
  });
  ...
  it('refuses arbitrary content or altered deterministic artifact input', async () => {
    ...
    const result = await runCanonicalQueueEntryWriterProbe({ projectRoot, execute: true, artifact: altered });
    assert.equal(result.status, 'blocked');
  ...
  it('captures bounded git evidence from injected pre-write and post-write readbacks', async () => {
    ...
    const preWriteEvidence = {
      git_status_short: { stdout: ALLOWED_PRE_EXISTING_TASK_CHANGES.map((file) => `?? ${file}`).join('\n') },
      git_diff_name_only: { stdout: '' },
      git_diff_cached_name_only: { stdout: '' }
    };
    const postWriteEvidence = {
      git_status_short: { stdout: [...ALLOWED_PRE_EXISTING_TASK_CHANGES, TARGET_PATH].map((file) => `?? ${file}`).join('\n') },
      git_diff_name_only: { stdout: TARGET_PATH },
      git_diff_cached_name_only: { stdout: '' }
    };
    const result = await runCanonicalQueueEntryWriterProbe({ projectRoot, execute: true, preWriteEvidence, postWriteEvidence });
    assert.equal(result.status, 'passed');
    assert.equal(result.pre_write_evidence, preWriteEvidence);
    assert.equal(result.post_write_evidence, postWriteEvidence);
    assert.equal(result.changed_file_reconciliation.matches, true);
    assert.deepEqual(result.changed_file_reconciliation.expected_files, [...ALLOWED_PRE_EXISTING_TASK_CHANGES, TARGET_PATH].sort());
  });
  ...
  it('refuses execute when pre-write evidence contains staged or unexpected files', async () => {
    ...
    const staged = await runCanonicalQueueEntryWriterProbe({
      projectRoot,
      execute: true,
      preWriteEvidence: { git_status_short: { stdout: 'A  scripts/agent/lib/canonical-queue-entry-writer-probe.mjs' } }
    });
  ...
  it('parses changed/staged files and reconciles expected changed files deterministically', () => {
    const evidence = {
      ...
      git_diff_cached_name_only: { stdout: 'scripts/agent/lib/canonical-queue-entry-writer-probe.mjs' }
    };
    assert.deepEqual(stagedFilesFromEvidence(evidence), ['scripts/agent/lib/canonical-queue-entry-writer-probe.mjs']);
    assert.deepEqual(changedFilesFromEvidence(evidence), ['reports/RALPH-045A_CANONICAL_QUEUE_ENTRY_PROBE_REPORT.md', 'scripts/agent/lib/canonical-queue-entry-writer-probe.mjs']);
  ...
  it('CLI parser requires explicit execute mode and refuses write/path/content operation flags', () => {
    ...
    for (const flag of ['--target', '--output', '--payload={}', '--content=x', '--append', '--truncate', '--delete', '--rename', '--move', '--commit', '--push', '--deploy', 'other.json']) {
