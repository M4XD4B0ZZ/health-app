...
function validConsumerDecision(overrides = {}) {
  return {
    ...consumeCanonicalQueueEntryProbeFromContent(JSON.stringify(validArtifact()), {
...
describe('execution-plan-preview', () => {
  ...
  it('fails closed for invalid JSON and missing consumer decision', () => {
    ...
    const missing = generateExecutionPlanPreviewFromConsumerDecision({
  ...
  it('fails closed for unsafe paths before preview generation', () => {
    const result = generateExecutionPlanPreview({
  ...
  it('fails closed for authority, execution, and mutation claims', () => {
    assert.equal(
      generateExecutionPlanPreviewFromConsumerDecision(
        validConsumerDecision({ runtime_authority: true }),
      ).decision,
      PREVIEW_DECISIONS.BLOCKED_AUTHORITY_CLAIM,
    );
    assert.equal(
      generateExecutionPlanPreviewFromConsumerDecision(
        validConsumerDecision({ queue_execution: true }),
      ).decision,
      PREVIEW_DECISIONS.BLOCKED_EXECUTION_CLAIM,
    );
    assert.equal(
      generateExecutionPlanPreviewFromConsumerDecision(
        validConsumerDecision({ writes_performed: true }),
      ).decision,
      PREVIEW_DECISIONS.BLOCKED_MUTATION_CLAIM,
    );
  });

  it('fails closed for wrong artifact type from JSON artifact input', () => {
    const result = generateExecutionPlanPreviewFromJson(
      JSON.stringify(validArtifact({ artifact_type: 'sandbox_queue_entry' })),
    );
  ...
  it('CLI parser remains read-only and refuses execution, mutation, and output flags', () => {
    ...
    assert.equal(
      parseArgs(['node', 'cli', '--input-file', DEFAULT_ARTIFACT_PATH]).inputPath,
      DEFAULT_ARTIFACT_PATH,
    );
    assert.equal(
      parseArgs(['node', 'cli', `--input-file=${DEFAULT_ARTIFACT_PATH}`]).inputPath,
