...
function changeSet(overrides = {}) {
  return {
    ...
    constraints: {
      ...
      forbidden_files: ['.env', '.env.*', 'package.json', 'package-lock.json', 'src/**', 'supabase/**'],
      expected_outputs: ['scripts/agent/example.mjs'],
      max_files_changed: 2,
      max_diff_lines: 120
    },
    changes: [
      { path: 'scripts/agent/example.mjs', change_type: 'modified', additions: 20, deletions: 5, binary: false, generated: false }
    ],
    ...overrides
  };
}
...
test('file outside allowed_files produces would_block with scope_violation', () => {
  const simulation = buildChangeDiffSimulation(changeSet({ changes: [
...
test('forbidden file produces would_block', () => {
  const simulation = buildChangeDiffSimulation(changeSet({
    ...
    changes: [{ path: 'src/domain/example.ts', change_type: 'modified', additions: 1, deletions: 0 }]
  }));
  assert.equal(simulation.disposition, 'would_block');
...
test('threshold exceeded produces would_block', () => {
  const simulation = buildChangeDiffSimulation(changeSet({ changes: [
    { path: 'scripts/agent/example.mjs', change_type: 'modified', additions: 121, deletions: 0 }
  ] }));
  assert.equal(simulation.disposition, 'would_block');
...
test('review trigger produces would_require_review', () => {
  const simulation = buildChangeDiffSimulation(changeSet({ changes: [
    { path: 'docs/example.md', change_type: 'deleted', additions: 0, deletions: 10 }
  ] }));
...
test('invalid input produces would_block', () => {
  const simulation = buildChangeDiffSimulation({ changes: [{ path: '../escape.md', change_type: 'modified' }] });
  assert.equal(simulation.disposition, 'would_block');
...
test('category classification flags documentation and dependency categories', () => {
  const doc = classifyChangedFile({ path: 'docs/readme.md', change_type: 'modified' }, changeSet().constraints);
  assert.equal(doc.validation_category, 'documentation-only');
  const dependency = classifyChangedFile({ path: 'package.json', change_type: 'modified' }, { ...changeSet().constraints, allowed_files: ['package.json'] });
...
test('CLI argument parser rejects execution and write flags', () => {
  for (const flag of ['--execute', '--worker', '--run-worker', '--invoke-worker', '--adapter', '--invoke-adapter', '--provider', '--model', '--execute-prompt', '--apply-diff', '--write-changes', '--validate', '--run-validation', '--write-report', '--write-run-log', '--output', '--commit', '--push', '--stage']) {
...
test('helper evaluations and pattern matching are deterministic', () => {
  ...
  const fileEvaluations = [classifyChangedFile({ path: 'unexpected.md', change_type: 'modified' }, changeSet().constraints)];
  assert.equal(evaluateScope(fileEvaluations).scope_violation_detected, true);
  assert.equal(evaluateThresholds([{ additions: 2, deletions: 3 }], { max_files_changed: 1, max_diff_lines: 5 }).threshold_exceeded, false);
});