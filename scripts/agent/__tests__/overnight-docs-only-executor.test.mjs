...
function input(pathValue = 'reports/example.md', sourceOverrides = {}, operationOverrides = {}) {
  return {
    ralph_034r_source: source(sourceOverrides),
    operation: {
...
function tempRoot() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ralph-034t-'));
}
...
test('write mode creates exactly one direct Markdown file in temp root', () => {
  ...
  assert.equal(
    fs.readFileSync(path.join(root, 'reports/example.md'), 'utf8'),
    '# Example\n\nTemp-root only.\n',
  );
...
test('write mode refuses BOM-prefixed Markdown content and writes no file', () => {
  ...
  const result = executeDocsOnlyOperation(
    input('reports/example.md', {}, { content: '\uFEFF# Example\n\nTemp-root only.\n' }),
...
test('rejects wrong RALPH-034R phase/mode/disposition', () => {
  assert.ok(
    executeDocsOnlyOperation(
      input('reports/example.md', { phase: 'RALPH-034Q' }),
    ).reason_codes.includes('invalid_source_phase'),
  );
  assert.ok(
    executeDocsOnlyOperation(input('reports/example.md', { mode: 'wrong' })).reason_codes.includes(
      'invalid_source_mode',
    ),
  );
  assert.ok(
    executeDocsOnlyOperation(
      input('reports/example.md', { execution_capability_disposition: 'blocked_for_execution' }),
    ).reason_codes.includes('source_not_eligible'),
  );
});
...
test('rejects source authorization claims and nonzero counters', () => {
  ...
  assert.ok(
    executeDocsOnlyOperation(
      input('reports/example.md', { execution_authorization: auth }),
    ).reason_codes.includes('source_claims_authorization'),
  );
  const plan = { ...source().execution_plan, validation_commands_executed: 1 };
  assert.ok(
    executeDocsOnlyOperation(
      input('reports/example.md', { execution_plan: plan }),
    ).reason_codes.includes('nonzero_source_counter'),
  );
});
...
test('CLI parser rejects forbidden flags', () => {
  for (const flag of [
    ...
    assert.throws(() => parseArgs(['node', 'cli', 'input.json', flag]), /forbidden/, flag);
});
