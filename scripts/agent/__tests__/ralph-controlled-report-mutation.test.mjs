...
function tempRoot(t) {
  ...
  fs.writeFileSync(path.join(root, '.agent/config/protected-files.json'), JSON.stringify(protectedFixture), 'utf8');
...
test('dry-run writes no file', async (t) => {
  ...
  const result = await runControlledReportMutation({ projectRoot: root, preEvidence: evidence(), protectedConfig: protectedFixture });
...
test('explicit execute creates exactly one report in temp root', async (t) => {
  const root = tempRoot(t);
  const result = await runControlledReportMutation({ projectRoot: root, execute: true, preEvidence: evidence(), postEvidence: evidence(`?? ${TARGET_PATH}\n`, `${TARGET_PATH}\n`), protectedConfig: protectedFixture, allowedPreExistingChangedFiles: [] });
...
test('overwrite refusal', async (t) => {
  ...
  const result = await runControlledReportMutation({ projectRoot: root, execute: true, preEvidence: evidence(), protectedConfig: protectedFixture, allowedPreExistingChangedFiles: [] });
...
test('path traversal, absolute, drive-qualified, and alternate paths are refused', () => {
  for (const candidate of ['../reports/x.md', '/reports/x.md', 'C:\\reports\\x.md', 'reports/other.md']) {
...
test('protected target refusal using fixture protected config', async (t) => {
  const root = tempRoot(t);
  const fixture = { protected_patterns: { absolute_protection: { patterns: [TARGET_PATH] }, conditional_protection: { patterns: [] } }, approval_required_patterns: { patterns: [] } };
  const classified = classifyProtectedTarget(TARGET_PATH, fixture);
  assert.equal(classified.target.protected, true);
  const result = await runControlledReportMutation({ projectRoot: root, execute: true, preEvidence: evidence(), protectedConfig: fixture, allowedPreExistingChangedFiles: [] });
...
test('staged files refusal using mocked evidence', async (t) => {
  const root = tempRoot(t);
  const result = await runControlledReportMutation({ projectRoot: root, execute: true, preEvidence: evidence(`M  ${TARGET_PATH}\n`, '', `${TARGET_PATH}\n`), protectedConfig: protectedFixture, allowedPreExistingChangedFiles: [] });
  assert.equal(result.status, 'blocked');
...
test('unexpected dirty working tree refusal using mocked evidence', async (t) => {
  const root = tempRoot(t);
  const result = await runControlledReportMutation({ projectRoot: root, execute: true, preEvidence: evidence(' M ROADMAP.md\n', 'ROADMAP.md\n'), protectedConfig: protectedFixture, allowedPreExistingChangedFiles: [] });
  assert.equal(result.status, 'blocked');
