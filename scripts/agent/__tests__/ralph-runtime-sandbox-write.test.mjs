...
import { executeSandboxRuntimeStateWrite, formatSandboxRuntimeStateWritePretty, SANDBOX_RUNTIME_STATE_PATH, validateSandboxRuntimeStatePath, validateSandboxRuntimeStatePayload } from '../lib/ralph-runtime-sandbox-write.mjs';
import { parseArgs } from '../ralph-runtime-sandbox-write.mjs';

function tempRoot() { return fs.mkdtempSync(path.join(os.tmpdir(), 'ralph-035b-')); }
function sandboxTarget(root) { return path.join(root, ...SANDBOX_RUNTIME_STATE_PATH.split('/')); }
...
test('explicit write creates exactly one fixed sandbox JSON file in temp root', () => {
  ...
  assert.deepEqual(result.readback_validations, [{ path: SANDBOX_RUNTIME_STATE_PATH, schema_exact: true, payload: { simulated: true } }]);
...
test('rejects traversal, absolute, drive-qualified, and alternative output paths', () => {
  for (const file of ['.agent/runtime/sandbox/../ralph-035b-simulated-state.json', '../.agent/runtime/sandbox/ralph-035b-simulated-state.json', '/tmp/ralph-035b-simulated-state.json', 'C:/tmp/ralph-035b-simulated-state.json', '.agent/runtime/sandbox/other.json', 'tasks/task-state.json', 'runs/current-run.json', 'validation/validation-results.jsonl', 'review/review-results.jsonl', 'handoffs/latest-handoff.md', 'ROADMAP.md']) {
    assert.equal(validateSandboxRuntimeStatePath(file).valid, false, file);
    const root = tempRoot();
    const result = executeSandboxRuntimeStateWrite({ executionRoot: root, writeSandboxState: true, path: file });
...
test('rejects alternative and authority-like payloads', () => {
  for (const payload of [{ simulated: false }, { simulated: true, extra: true }, { task_id: 'RALPH-035B', simulated: true }, { run_id: 'run', simulated: true }, { status: 'done' }, ['not-object'], null]) {
    const validation = validateSandboxRuntimeStatePayload(payload);
    assert.equal(validation.valid, false, JSON.stringify(payload));
    const root = tempRoot();
    const result = executeSandboxRuntimeStateWrite({ executionRoot: root, writeSandboxState: true, payload });
...
test('CLI parser requires explicit write flag and rejects output/path/payload/command flags', () => {
  assert.deepEqual(parseArgs(['node', 'cli']), { pretty: false, help: false, writeSandboxState: false });
  assert.deepEqual(parseArgs(['node', 'cli', '--pretty', '--write-sandbox-state']), { pretty: true, help: false, writeSandboxState: true });
  for (const flag of ['--output', '--path', '--payload', '--append', '--truncate', '--overwrite', '--worker', '--adapter', '--provider', '--model', '--prompt', '--validate', '--review', '--approve', '--stage', '--commit', '--push', '--deploy', '--write-runtime', '--write-evidence', '--write-governance', '--write-product', '--write-package', '--write-handoff', '--write-tasks', '--write-runs', '--write-validation', '--write-review']) assert.throws(() => parseArgs(['node', 'cli', flag]), /forbidden/, flag);
  assert.throws(() => parseArgs(['node', 'cli', 'some-output.json']), /Unexpected positional argument/, 'positional output path');
});
...
test('pretty output states sandbox boundaries and non-authorization language', () => {
  ...
  assert.match(pretty, /no canonical runtime\/evidence\/governance\/product\/package\/handoff mutation/);
