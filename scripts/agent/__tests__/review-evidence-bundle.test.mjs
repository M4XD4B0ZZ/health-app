...
function readbacks(status = ' M scripts/agent/lib/review-evidence-bundle.mjs\n') {
  return {
    ...
    git_log_last_oneline: command('git_log_last_oneline', 'c6e1aa9 chore(roadmap): register review evidence bundle task\n'),
    git_diff_stat: command('git_diff_stat', ' scripts/agent/lib/review-evidence-bundle.mjs | 1 +\n'),
    git_diff_name_only: command('git_diff_name_only', 'scripts/agent/lib/review-evidence-bundle.mjs\n'),
...
test('Markdown bundle contains expected sections', async () => {
  const bundle = await generateReviewEvidenceBundle({ taskId: 'RALPH-037B', gitReadbacks: readbacks(), protectedConfig: protectedFixture, claimedChangedFiles: ['scripts/agent/lib/review-evidence-bundle.mjs'] });
...
test('changed-file classification works', () => {
  const classified = classifyChangedFiles(['src/domain/x.ts', 'ROADMAP.md', 'package.json', 'scripts/agent/x.mjs']);
...
test('protected and approval-required detection works with fixture protected config', () => {
  const result = classifyProtectedFiles(['.env', 'package.json', 'supabase/migrations/1.sql'], protectedFixture);
...
test('claim-vs-actual mismatch blocks commit readiness', async () => {
  const bundle = await generateReviewEvidenceBundle({ taskId: 'RALPH-037B', gitReadbacks: readbacks(), protectedConfig: protectedFixture, claimedChangedFiles: ['different.file'], requiredChecks: [{ id: 'check', status: 'passed', evidence: 'ok' }] });

  assert.equal(bundle.claim_reconciliation.matches, false);
  assert.equal(bundle.commit_readiness.ready, false);
  assert.ok(bundle.commit_readiness.blocking_findings.some((finding) => finding.code === 'claimed_changed_files_mismatch'));
});
...
test('staged files when disallowed block commit readiness', async () => {
  ...
  const bundle = await generateReviewEvidenceBundle({ taskId: 'RALPH-037B', gitReadbacks: staged, protectedConfig: protectedFixture, claimedChangedFiles: ['scripts/agent/lib/review-evidence-bundle.mjs'], requiredChecks: [{ id: 'check', status: 'passed', evidence: 'ok' }] });

  assert.equal(bundle.commit_readiness.ready, false);
  assert.ok(bundle.commit_readiness.blocking_findings.some((finding) => finding.code === 'staged_files_disallowed'));
});

test('missing required evidence blocks commit readiness', async () => {
  const incomplete = readbacks();
  delete incomplete.git_diff_stat;
  const bundle = await generateReviewEvidenceBundle({ taskId: 'RALPH-037B', gitReadbacks: incomplete, protectedConfig: protectedFixture, claimedChangedFiles: ['scripts/agent/lib/review-evidence-bundle.mjs'] });

  assert.equal(bundle.commit_readiness.ready, false);
  assert.ok(bundle.commit_readiness.blocking_findings.some((finding) => finding.code === 'missing_required_git_evidence'));
  assert.ok(bundle.commit_readiness.blocking_findings.some((finding) => finding.code === 'missing_required_verification_evidence') === false);
});
...
test('output truncation metadata is explicit and blocks required evidence readiness', async () => {
  ...
  const bundle = await generateReviewEvidenceBundle({ taskId: 'RALPH-037B', gitReadbacks: truncated, protectedConfig: protectedFixture, claimedChangedFiles: ['scripts/agent/lib/review-evidence-bundle.mjs'], requiredChecks: [{ id: 'check', status: 'passed', evidence: 'ok' }] });

  assert.equal(bundle.git_readbacks.git_status_short.stdout_truncated, true);
  assert.ok(bundle.commit_readiness.blocking_findings.some((finding) => finding.code === 'required_evidence_truncated'));
});

test('missing required verification evidence blocks commit readiness', async () => {
  const bundle = await generateReviewEvidenceBundle({ taskId: 'RALPH-037B', gitReadbacks: readbacks(), protectedConfig: protectedFixture, claimedChangedFiles: ['scripts/agent/lib/review-evidence-bundle.mjs'], requiredChecks: [{ id: 'node_test' }] });

  assert.equal(bundle.commit_readiness.ready, false);
  assert.ok(bundle.verification.missing_required_checks.includes('node_test'));
  assert.ok(bundle.commit_readiness.blocking_findings.some((finding) => finding.code === 'missing_required_verification_evidence'));
});
...
test('dry-run/default writes no files', async (t) => {
  ...
  const bundle = await generateReviewEvidenceBundle({ projectRoot: root, taskId: 'RALPH-037B', gitReadbacks: readbacks(), protectedConfig: protectedFixture, claimedChangedFiles: ['scripts/agent/lib/review-evidence-bundle.mjs'] });
...
test('malformed protected config blocks commit readiness', async () => {
  const bundle = await generateReviewEvidenceBundle({ taskId: 'RALPH-037B', gitReadbacks: readbacks(), protectedConfig: { bad: true }, claimedChangedFiles: ['scripts/agent/lib/review-evidence-bundle.mjs'], requiredChecks: [{ id: 'check', status: 'passed', evidence: 'ok' }] });

  assert.equal(bundle.commit_readiness.ready, false);
  assert.ok(bundle.commit_readiness.blocking_findings.some((finding) => finding.code === 'malformed_protected_config'));
});

test('task ID missing when required blocks commit readiness', async () => {
  const bundle = await generateReviewEvidenceBundle({ gitReadbacks: readbacks(), protectedConfig: protectedFixture, claimedChangedFiles: ['scripts/agent/lib/review-evidence-bundle.mjs'], requiredChecks: [{ id: 'check', status: 'passed', evidence: 'ok' }] });

  assert.equal(bundle.commit_readiness.ready, false);
  assert.ok(bundle.commit_readiness.blocking_findings.some((finding) => finding.code === 'task_id_missing'));
});
