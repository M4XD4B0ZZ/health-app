...
describe('sandbox-promotion-proposal-writer-probe', () => {
  ...
  it('refuses overwrite via create-only semantics', async () => {
    const projectRoot = tempRoot();
    const first = await runSandboxPromotionProposalWriterProbe({
      projectRoot,
      execute: true,
      proposal: validProposal(),
    });
    const second = await runSandboxPromotionProposalWriterProbe({
      projectRoot,
      execute: true,
      proposal: validProposal(),
    });
  ...
  it('refuses symlink escapes in target path components', async () => {
    ...
    fs.symlinkSync(
      outside,
      path.join(projectRoot, '.agent', 'overnight', 'promotion-proposals'),
      'junction',
    );
    const result = await runSandboxPromotionProposalWriterProbe({
      projectRoot,
  ...
  it('fails closed for malformed or non-RALPH-044A proposal shapes', () => {
    ...
    assert.ok(
      validatePromotionProposalInput(validProposal({ generator: 'other' })).findings.some(
        (entry) => entry.code === 'source_generator_mismatch',
      ),
    );
    assert.ok(
      validatePromotionProposalInput(validProposal({ proposal_created: false })).findings.some(
        (entry) => entry.code === 'proposal_not_created',
      ),
    );
  });
  ...
  it('fails closed when writes/stdout/authority/source markers are invalid', () => {
    assert.ok(
      validatePromotionProposalInput(validProposal({ writes_performed: true })).findings.some(
        (entry) => entry.code === 'proposal_writes_performed_not_false',
      ),
    );
    assert.ok(
      validatePromotionProposalInput(validProposal({ stdout_only: false })).findings.some(
        (entry) => entry.code === 'proposal_stdout_only_not_true',
      ),
    );
    assert.ok(
      validatePromotionProposalInput(
        validProposal({
          authority_flags: { ...validProposal().authority_flags, queue_execution: true },
        }),
      ).findings.some((entry) => entry.code === 'proposal_authority_flags_not_all_false'),
    );
    assert.ok(
      validatePromotionProposalInput(
        validProposal({ source_summary: { sandbox: false, non_authoritative: true } }),
      ).findings.some((entry) => entry.code === 'source_sandbox_marker_missing'),
    );
  });
  ...
  it('CLI parser requires explicit write mode and refuses write/path operation flags', () => {
    ...
    assert.equal(
      parseArgs([
        ...
        '--proposal-json',
        '{}',
        '--execute-canonical-promotion-proposal-probe',
      ]).execute,
      true,
    );
    for (const flag of [
