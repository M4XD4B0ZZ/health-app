...
export function parseArgs(argv) {
  ...
  if (args.format !== 'json' && args.format !== 'markdown')
    throw new Error(`Invalid format: ${args.format}. Must be json or markdown`);
  return args;
}

async function readSource(args) {
  const supplied = [
    args.eligibilityJson,
    args.eligibilityFile,
    args.artifactJson,
    args.artifactFile,
  ].filter((value) => value !== null && value !== undefined);
  if (supplied.length !== 1)
    throw new Error(
      'Specify exactly one of --eligibility-json, --eligibility-file, --artifact-json, or --artifact-file',
    );
  if (args.eligibilityJson)
    return { kind: 'eligibility', value: parsePromotionJson(args.eligibilityJson) };
  if (args.eligibilityFile)
    return { kind: 'eligibility', value: await readPromotionInputFile(args.eligibilityFile) };
...
async function main() {
  try {
    ...
    if (args.unknownFlags.length > 0)
      throw new Error(`Unknown flags: ${args.unknownFlags.join(', ')}`);
    if (args.positionalArgs.length > 0)
      throw new Error(`Unexpected positional arguments: ${args.positionalArgs.join(', ')}`);
    const source = await readSource(args);
    const proposal =
      source.kind === 'artifact'
        ? buildSandboxPromotionProposalFromArtifact(source.value)
        : buildSandboxPromotionProposal(source.value);
    const output =
      args.format === 'markdown'
        ? formatSandboxPromotionProposalMarkdown(proposal)
...
if (
  ...
  process.argv[1]?.endsWith('generate-sandbox-promotion-proposal.mjs')
) {
  main();
}
