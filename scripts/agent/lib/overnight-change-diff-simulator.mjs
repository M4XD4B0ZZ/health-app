...
function classifyValidationCategory(pathText) {
  ...
  if (/^(package\.json|package-lock\.json|npm-shrinkwrap\.json)$/.test(path)) return 'dependency-change';
  if (/^supabase\//.test(path)) return 'edge-supabase';
  if (/^src\//.test(path) || /^(App\.tsx|Apptest\.tsx|index\.ts|app\.json|babel\.config\.js|jest\.config\.js|tsconfig\.json)$/.test(path)) return 'product-runtime-code';
  if (/^scripts\/agent\/.*\.mjs$/.test(path)) return 'governance-script-only';
  if (/^(tasks\/|runs\/|validation\/validation-results\.jsonl$|review\/review-results\.jsonl$)/.test(path)) return 'runtime-or-evidence-state';
  if (/(^__tests__\/|\/__tests__\/|\.test\.[cm]?js$|\.spec\.[cm]?js$)/i.test(path)) return 'test-only';
  if (/^(AGENTS\.md|SSOK\.md|VERIFY\.md|ROADMAP\.md)$/.test(path) || /^\.governance\/.*\.md$/.test(path)) return 'governance-only';
...
function validateConstraints(constraints = {}) {
  ...
  if (!Array.isArray(constraints.allowed_files) || constraints.allowed_files.length === 0) findings.push('constraints.allowed_files must be a non-empty array');
  if (!Array.isArray(constraints.forbidden_files)) findings.push('constraints.forbidden_files must be an array');
  for (const field of ['max_files_changed', 'max_diff_lines']) {
    if (!Number.isInteger(constraints[field]) || constraints[field] < 0) findings.push(`constraints.${field} must be a non-negative integer`);
  }
  return findings;
}
...
export function classifyChangedFile(change = {}, constraints = {}) {
  ...
  const additions = Number.isInteger(change.additions) && change.additions >= 0 ? change.additions : 0;
  const deletions = Number.isInteger(change.deletions) && change.deletions >= 0 ? change.deletions : 0;
  ...
  if (['deleted', 'renamed', 'copied', 'mode_changed', 'binary_changed', 'unknown'].includes(changeType) || change.binary === true) reasonCodes.push('review_policy_trigger');
  if (['product-runtime-code', 'edge-supabase', 'dependency-change', 'runtime-or-evidence-state', 'protected_secret_or_env', 'unknown-or-product-runtime-code'].includes(category)) reasonCodes.push('category_escalation');
  ...
  evaluation.review_required = evaluation.blocking || evaluation.severity === 'review' || evaluation.reason_codes.includes('category_escalation');
  return evaluation;
}

export function evaluateScope(fileEvaluations = []) {
  const outsideAllowedFiles = fileEvaluations.filter((entry) => entry.scope_violation).map((entry) => entry.normalized_path);
  const forbiddenFiles = fileEvaluations.filter((entry) => entry.forbidden_file_violation).map((entry) => entry.normalized_path);
  const protectedFiles = fileEvaluations.filter((entry) => entry.protected_file_violation).map((entry) => entry.normalized_path);
  return {
...
export function simulateReviewGate(fileEvaluations = [], thresholdEvaluation = {}) {
  ...
  for (const entry of fileEvaluations) {
    for (const code of entry.reason_codes) {
      ...
      if (['scope_violation', 'forbidden_file', 'protected_file', 'invalid_input'].includes(code)) blockingReasons.add(code);
    }
  }
  if (fileEvaluations.length > 1) reviewTriggers.add('review_policy_trigger');
  if (thresholdEvaluation.review_policy_large_diff_trigger) reviewTriggers.add('review_policy_trigger');
  ...
  return {
    ...
    review_gate_disposition: blockingReasons.size > 0 ? 'block_before_review' : reviewTriggers.size > 0 ? 'requires_manual_review' : 'manual_review_required_for_future_worker_changes',
...
function summarizeChangeTypes(fileEvaluations) {
  const summary = { created: 0, modified: 0, deleted: 0, renamed: 0, copied: 0, mode_changed: 0, binary_changed: 0, unknown: 0 };
  for (const entry of fileEvaluations) summary[entry.change_type] = (summary[entry.change_type] || 0) + 1;
  return summary;
}

function topLevelDisposition(inputFindings, scopeEvaluation, thresholdEvaluation, reviewGateSimulation) {
  if (inputFindings.length > 0) return 'would_block';
  if (scopeEvaluation.protected_file_violation_detected || scopeEvaluation.forbidden_file_violation_detected || scopeEvaluation.scope_violation_detected) return 'would_block';
...
function topLevelReasonCodes(inputFindings, scopeEvaluation, thresholdEvaluation, reviewGateSimulation) {
  const codes = new Set();
...
export function buildChangeDiffSimulation(input = {}, options = {}) {
  ...
  const disposition = topLevelDisposition(inputFindings, scopeEvaluation, thresholdEvaluation, reviewGateSimulation);
  const reasonCodes = topLevelReasonCodes(inputFindings, scopeEvaluation, thresholdEvaluation, reviewGateSimulation);
  const validationCategories = [...new Set(fileEvaluations.map((entry) => entry.validation_category))].sort();
  ...
  return {
    ...
    validation_category_simulation: {
      ...
      category_escalation_detected: fileEvaluations.some((entry) => entry.reason_codes.includes('category_escalation')),
    ...
    recommended_human_actions: disposition === 'would_pass'
...
export function formatChangeDiffSimulationPretty(simulation) {
  ...
  lines.push('Execution: planning-only change/diff simulation; no queued task execution; no prompt execution; no validation commands; no worker invocation; no adapter invocation; no provider/model invocation; no network activity; no runtime/evidence mutation; no file writes; no commit; no push.');
  lines.push('');
  lines.push('Summary:');
  ...
  for (const entry of simulation.file_evaluations) {
    lines.push(`- ${entry.normalized_path || '(missing)'} ${entry.change_type} severity=${entry.severity} reasons=${entry.reason_codes.join(',') || '(none)'}`);
  }
  lines.push('');
