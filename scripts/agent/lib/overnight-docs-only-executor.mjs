...
const REQUIRED_TRUE_SOURCE_FLAGS = Object.freeze([
  'planning_only',
  'non_authoritative',
  'non_mutating',
...
function intendedFiles(source = {}) {
  const fromScope = isPlainObject(source.hypothetical_task_scope)
    ? source.hypothetical_task_scope.intended_changed_files
    : null;
  const files = Array.isArray(fromScope)
    ? fromScope
    : Array.isArray(source.intended_changed_files)
      ? source.intended_changed_files
      : null;
...
export function validateDocsOnlyPath(pathValue) {
  ...
  if (requestedPath.startsWith('/') || /^[A-Za-z]:\//.test(requestedPath))
    return {
      ...
      path: requestedPath,
      code: 'invalid_path',
      message: 'Absolute or drive-letter paths are forbidden.',
    };
  ...
  if (requestedPath === '.env' || requestedPath.startsWith('.env.'))
    return {
      valid: false,
      path: requestedPath,
      code: 'forbidden_path',
      message: '.env paths are forbidden.',
    };
  if (
    [
      'package.json',
    ...
    return {
      ...
      path: requestedPath,
      code: 'forbidden_path',
      message: 'Root/high-authority files are forbidden.',
    };
  if (
    [
      '.agent/',
    ...
    return {
      ...
      path: requestedPath,
      code: 'forbidden_path',
      message: 'Forbidden scope for docs-only executor.',
    };
  if (!/^(docs|plans|reports)\/[^/]+\.md$/.test(requestedPath))
    return {
      valid: false,
      path: requestedPath,
      code: 'not_direct_low_authority_markdown',
      message:
        'Only direct docs/<file>.md, plans/<file>.md, or reports/<file>.md paths are allowed.',
    };
  return {
    valid: true,
    path: requestedPath,
    code: 'allowed_direct_docs_plans_reports_markdown',
    message: 'Allowed direct low-authority Markdown path.',
  };
}
...
export function validateSource(source = {}) {
  ...
  if (!isPlainObject(source))
    return {
      ...
      blocking_findings: [
        finding(
          'blocking',
          'missing_ralph_034r_source',
          'Input must include a RALPH-034R source object.',
        ),
      ],
    };
  if (source.phase !== SOURCE_EXECUTION_CAPABILITY_PHASE)
    blocking.push(
      finding(
        'blocking',
        'invalid_source_phase',
        `Expected source phase ${SOURCE_EXECUTION_CAPABILITY_PHASE}.`,
        { actual: source.phase ?? null },
      ),
    );
  if (source.mode !== REQUIRED_SOURCE_MODE)
    blocking.push(
      finding('blocking', 'invalid_source_mode', `Expected source mode ${REQUIRED_SOURCE_MODE}.`, {
        actual: source.mode ?? null,
      }),
    );
  if (source.execution_capability_disposition !== REQUIRED_SOURCE_DISPOSITION)
    blocking.push(
      finding(
        'blocking',
        'source_not_eligible',
        `Expected source disposition ${REQUIRED_SOURCE_DISPOSITION}.`,
        { actual: source.execution_capability_disposition ?? null },
      ),
    );
  if (source.valid !== true)
    blocking.push(
      finding('blocking', 'source_not_valid', 'Source RALPH-034R simulation must be valid true.', {
        actual: source.valid ?? null,
      }),
    );
  ...
  for (const field of REQUIRED_ZERO_SOURCE_COUNTERS)
    if (plan[field] !== 0)
      blocking.push(
        finding('blocking', 'nonzero_source_counter', `Source counter must remain zero: ${field}`, {
          field,
          actual: plan[field] ?? null,
        }),
      );
  for (const field of REQUIRED_FALSE_SOURCE_COUNTERS)
    if (plan[field] !== false)
      blocking.push(
        finding('blocking', 'unsafe_source_boolean', `Source boolean must remain false: ${field}`, {
          field,
          actual: plan[field] ?? null,
        }),
      );
  for (const field of REQUIRED_TRUE_SOURCE_FLAGS)
    if (safety[field] !== true)
      blocking.push(
        finding(
          'blocking',
          'missing_source_safety_flag',
          `Source safety flag must be true: ${field}`,
          { field, actual: safety[field] ?? null },
        ),
      );
  for (const field of SOURCE_AUTHORIZATION_FIELDS)
    if (source[field] === true || auth[field] === true)
      blocking.push(
        finding(
          'blocking',
...
export function validateOperation(operation, source) {
    ...
    return {
      ...
      blocking_findings: [
        finding(
          'blocking',
          'missing_operation',
          'Input must include exactly one operation object.',
        ),
      ],
    };
  if (operation.type !== SUPPORTED_OPERATION)
    blocking.push(
      finding('blocking', 'unsupported_operation', `Only ${SUPPORTED_OPERATION} is supported.`, {
        actual: operation.type ?? null,
      }),
    );
  const pathCheck = validateDocsOnlyPath(operation.path);
  if (!pathCheck.valid)
    blocking.push(finding('blocking', pathCheck.code, pathCheck.message, { path: pathCheck.path }));
  const content = operation.content;
  if (typeof content !== 'string' || content.length === 0)
    blocking.push(
      finding('blocking', 'invalid_content', 'Markdown content must be a non-empty string.'),
    );
  if (typeof content === 'string' && content.startsWith('\uFEFF'))
    blocking.push(
      finding(
        'blocking',
        'content_has_bom',
        'Markdown content must not start with a BOM; remove the leading BOM before docs-only write.',
      ),
    );
  if (typeof content === 'string' && content.length > 100000)
    blocking.push(
      finding(
        'blocking',
        'content_too_large',
        'Markdown content is too large for first docs-only capability.',
      ),
    );
  if (typeof content === 'string' && /[\u0000-\u0008\u000B\u000C\u000E-\u001F]/.test(content))
    blocking.push(
      finding(
        'blocking',
        'content_has_control_characters',
        'Markdown content contains forbidden control characters.',
      ),
    );
  ...
  if (sourceFiles.length !== 1)
    blocking.push(
      finding(
        'blocking',
        'source_file_count_not_one',
        'RALPH-034T supports exactly one intended changed file.',
        { source_files: sourceFiles },
      ),
    );
  if (requestedPath && sourceFiles.length === 1 && sourceFiles[0] !== requestedPath)
    blocking.push(
      finding(
        'blocking',
        'requested_path_mismatch',
        'Requested operation path must match RALPH-034R intended changed file.',
        { requested_path: requestedPath, source_files: sourceFiles },
      ),
    );
  const sourceFileScope = asArray(source?.file_scope_analysis).find(
    (entry) => normalizePath(entry?.path) === requestedPath,
  );
  if (sourceFileScope && sourceFileScope.classification !== 'eligible_low_authority_docs_markdown')
    blocking.push(
      finding(
        'blocking',
        'source_file_scope_not_eligible',
        'Matching RALPH-034R file_scope_analysis entry must be eligible low-authority Markdown.',
        { classification: sourceFileScope.classification ?? null },
      ),
    );
  return {
...
export function executeDocsOnlyOperation(input = {}, options = {}) {
  ...
  const blockingFindings = [
    ...sourceValidation.blocking_findings,
    ...operationValidation.blocking_findings,
  ];
  const plannedFiles = operationValidation.requested_path
    ? [operationValidation.requested_path]
    : [];
  const createdFiles = [];
  const refusedFiles = blockingFindings.map((entry) => entry.details?.path).filter(Boolean);
  let operationStatus =
    blockingFindings.length > 0 ? 'refused' : writeMode ? 'ready_to_write' : 'dry_run_planned';
  if (blockingFindings.length === 0 && writeMode) {
    try {
      const target = safeTargetPath(executionRoot, operationValidation.requested_path);
      if (fs.existsSync(target))
        throw new Error('Target file already exists; overwrite is refused.');
      ...
      blockingFindings.push(
        finding('blocking', 'write_failed', 'Docs-only write failed or was refused.', {
          message: error.message,
          path: operationValidation.requested_path,
        }),
      );
  ...
  return {
    ...
    safety_summary: {
      supervised_docs_only_executor: true,
      dry_run_by_default: true,
      writes_require_explicit_write_docs_only_flag: true,
      writes_limited_to_direct_docs_plans_reports_markdown: true,
      create_only: true,
      overwrite_refused: true,
      no_queued_task_execution: true,
...
export function formatDocsOnlyExecutorPretty(result) {
  const lines = [
    'RALPH-034T Supervised Docs-Only Executor',
    '',
  ...
  lines.push(
    '',
    'Execution: supervised docs-only create capability; dry-run by default; write requires --write-docs-only; no queued task execution; no validation execution; no review acceptance; no runtime/evidence mutation; no worker invocation; no adapter invocation; no provider/model invocation; no prompt execution; no staging; no commit; no push.',
    '',
