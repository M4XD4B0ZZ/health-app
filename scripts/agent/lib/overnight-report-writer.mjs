...
function normalizeFormats(formats = DEFAULT_FORMATS) {
  ...
  const normalized = asArray(input).map((format) => String(format || '').trim().toLowerCase()).filter(Boolean);
...
function truncateText(value, limit = DEFAULT_PREVIEW_CHARS) {
  ...
  if (text.length <= limit) return { preview: text, truncated: false, original_length: text.length, preview_length: text.length };
  return {
...
export function buildOvernightReportPayload(executorOutput, options = {}) {
  const generatedAt = reportTimestamp(options.generatedAt || options.now || new Date().toISOString());
  const safeQueueId = sanitizeReportQueueId(executorOutput?.queue_id, options);
  const commandResults = asArray(executorOutput?.command_execution?.results).map((result) => summarizeCommandResult(result, options));
  const failedCommands = commandResults.filter((result) => ['failed', 'timed_out', 'blocked'].includes(result.status));
  ...
  return {
    ...
    next_boundary: 'Review this non-authoritative operational report manually. Real queued task execution, worker invocation, runtime/evidence mutation, product work, commit, and push remain out of scope.',
...
export function formatOvernightReportMarkdown(reportPayload) {
  ...
  lines.push(`- Working tree clean before: ${reportPayload.repository?.working_tree_clean_before === true}`);
  lines.push(`- Working tree clean after: ${reportPayload.repository?.working_tree_clean_after === true}`);
  lines.push('- Repository metadata comes from validation executor preflight/final git status checks only.');
  lines.push('');
  lines.push('## Queue Summary');
  lines.push(`- Queue validation critical findings: ${asArray(reportPayload.queue_validation?.critical).length}`);
  lines.push(`- Queue validation warnings: ${asArray(reportPayload.queue_validation?.warnings).length}`);
  lines.push(`- Queue validation info findings: ${asArray(reportPayload.queue_validation?.info).length}`);
  lines.push('');
  ...
  lines.push(`- Ready for validation execution: ${reportPayload.validation_plan_summary?.ready_for_validation_execution === true}`);
  if (criticalFindings.length > 0) {
    lines.push('- Critical findings:');
    for (const finding of criticalFindings) lines.push(`  - [${finding.code || 'unknown'}] ${finding.message || ''}`);
  }
  lines.push('');
  ...
  for (const command of commands) {
    lines.push(`- ${command.command_id || '(unknown)'}: ${command.status || '(unknown)'}; exit=${command.exit_code}; duration_ms=${command.duration_ms}; timed_out=${command.timed_out}`);
  }
  lines.push('');
  lines.push('## Failed, Timed-Out, or Blocked Commands');
  if (failed.length === 0) lines.push('- None');
  for (const command of failed) lines.push(`- ${command.command_id || '(unknown)'}: ${command.status || '(unknown)'}`);
  lines.push('');
  lines.push('## Safety Invariants');
  lines.push(`- Queued tasks executed: ${reportPayload.execution_plan?.queued_tasks_executed || 0}`);
  lines.push(`- Worker invocations: ${reportPayload.execution_plan?.worker_invocations || 0}`);
  lines.push(`- Runtime state mutations: ${reportPayload.execution_plan?.runtime_state_mutations || 0}`);
  lines.push(`- Task commands executed: ${reportPayload.execution_plan?.task_commands_executed || 0}`);
  ...
  if (files.length === 0) lines.push('- None by formatter; report files are listed only after explicit write mode.');
  for (const file of files) lines.push(`- ${file}`);
  lines.push('');
  lines.push('## Human Decisions Required');
  if (actions.length === 0) lines.push('- Review report and decide whether a future task should proceed.');
  ...
  lines.push('- This report is non-authoritative operational output and is not authoritative runtime evidence.');
  lines.push('');
  return lines.join('\n');
}
...
export function writeOvernightReportBundle(executorOutput, options = {}) {
  ...
  for (const format of resolved.formats) {
    if (fs.existsSync(resolved.paths[format])) {
      throw new Error(`Refusing to overwrite existing overnight report: ${relativeReportPath(resolved.projectRoot, resolved.paths[format])}`);
    }
  }
  ...
  finalPayload.files_written = resolved.formats.map((format) => relativeReportPath(resolved.projectRoot, resolved.paths[format]));
  ...
  for (const format of resolved.formats) {
    ...
    writes.push({ format, path: resolved.paths[format], relative_path: relativeReportPath(resolved.projectRoot, resolved.paths[format]) });
  }
