...
export function validateValidationApprovalGateInput(input = {}) {
  ...
  if (!isPlainObject(input)) {
    blockingFindings.push(
      createFinding(
        'blocking',
        'invalid_input',
        'Input must be a single RALPH-034N validation approval-gate simulation object.',
      ),
    );
    return { valid: false, blocking_findings: blockingFindings };
  }

  if (input.phase !== SOURCE_VALIDATION_APPROVAL_GATE_PHASE)
    blockingFindings.push(
      createFinding(
        'blocking',
        'invalid_source_phase',
        `Expected source phase ${SOURCE_VALIDATION_APPROVAL_GATE_PHASE}.`,
  ...
  if (!Array.isArray(input.hypothetical_validation_requirements))
    blockingFindings.push(
      createFinding(
        'blocking',
        'missing_hypothetical_validation_requirements',
        'Input is missing required hypothetical_validation_requirements array.',
      ),
    );
  if (!isPlainObject(input.approval_consideration))
    blockingFindings.push(
      createFinding(
        'blocking',
        'missing_approval_consideration',
        'Input is missing required approval_consideration object.',
      ),
    );
...
export function evaluateSourceSafetyInvariants(source = {}) {
  ...
  const approvalConsideration = isPlainObject(source.approval_consideration)
    ? source.approval_consideration
    : {};
  ...
  for (const field of SOURCE_AUTHORIZATION_FIELDS) {
    if (source[field] === true || approvalConsideration[field] === true)
      blockingFindings.push(
        createFinding(
          'blocking',
  ...
  if (
    approvalConsideration.not_approval !== true ||
    approvalConsideration.not_review_evidence !== true ||
    approvalConsideration.not_validation_evidence !== true
  ) {
    blockingFindings.push(
      createFinding(
        'blocking',
        'source_claims_evidence_or_approval_authority',
        'Source must explicitly remain non-approval and non-evidence.',
      ),
    );
  }
...
function evaluateBlockedRequirements(requirements = []) {
  ...
  for (const entry of requirements) {
    if (
      BLOCKED_REQUIREMENT_IDS.has(entry?.requirement_id) ||
      BLOCKED_CATEGORIES.has(entry?.category)
    ) {
      blockingFindings.push(
        createFinding(
          'blocking',
          'blocked_transition_requirement',
          'Source validation requirement fails closed before transition planning.',
          { requirement_id: entry?.requirement_id || null, category: entry?.category || null },
        ),
      );
    }
  }
...
export function buildHypotheticalRuntimeTransitions(source = {}) {
  ...
  const validationRequirementSource =
    requiredChecks.length > 0
      ? `RALPH-034N hypothetical requirements: ${requiredChecks.join(', ')}`
      : 'RALPH-034N hypothetical validation requirements';
  ...
  return [
    runtimeTransition({
      ...
      authority:
        'runs/current-run.json and runs/run-history.jsonl via future authorized transition writer',
    ...
    runtimeTransition({
      ...
      authority:
        'tasks/task-state.json and tasks/task-history.jsonl via future authorized transition writer',
      fromStatus: 'needs_validation',
      toStatus: 'needs_review',
      requiredActor: 'validator',
      requiredEvidence: ['passing_validation_or_docs_readback'],
      rationale:
        'A future task could only move to needs_review after validation/readback evidence exists.',
    }),
    runtimeTransition({
      ...
      rationale:
        'A future run could only complete after human review acceptance is recorded by an authorized evidence path.',
    }),
    runtimeTransition({
      ...
      rationale:
        'A future task could only be marked done after both validation evidence and human review acceptance exist.',
    }),
  ];
}
...
export function buildHypotheticalEvidenceTransitions(source = {}) {
  ...
  return [
    evidenceTransition({
      ...
      authority:
        'validation/validation-results.jsonl via future authorized validation evidence writer',
      requiredRecordType: 'validation.result',
      requiredSource:
        requiredChecks.length > 0
          ? requiredChecks.join(', ')
          : 'VERIFY.md required checks identified by RALPH-034N',
      rationale:
        'RALPH-034N identified validation requirements, but did not execute or satisfy them. A future workflow would need canonical validation evidence before completion transitions.',
    }),
    evidenceTransition({
      transitionId: 'review_evidence_required',
      stateDomain: 'review_evidence',
      authority:
        'review/review-results.jsonl or canonical review acceptance evidence path via future authorized reviewer',
      requiredRecordType: 'review.accepted',
      requiredSource: '.governance/REVIEW_POLICY.md human review acceptance gate',
      rationale:
        'Human review remains mandatory before approval-path runtime completion transitions can be considered.',
    }),
    evidenceTransition({
    ...
    evidenceTransition({
      ...
      rationale:
        'A future approved workflow would need a handoff documenting checks, results, risks, and human-review status.',
    }),
  ];
}
...
function evaluateTransitionDisposition(source, inputEvaluation, safetyEvaluation) {
  ...
  if (safetyEvaluation.blocking_findings.length > 0)
    return {
      disposition: 'no_future_workflow_consideration',
      blockingFindings,
  ...
  if (sourceDisposition === 'invalid_input') {
    blockingFindings.push(
      createFinding('blocking', 'source_invalid_input', 'Source RALPH-034N simulation is invalid.'),
    );
    reasonCodes.add('invalid_input');
    return { disposition: 'invalid_input', blockingFindings, reasonCodes: [...reasonCodes] };
  }
  if (sourceDisposition === 'no_future_approval_consideration') {
    blockingFindings.push(
      createFinding(
        'blocking',
        'source_no_future_approval_consideration',
        'Source RALPH-034N simulation blocks future approval consideration.',
      ),
    );
  ...
  if (sourceDisposition === 'blocked_before_validation') {
    blockingFindings.push(
      createFinding(
        'blocking',
        'source_blocked_before_validation',
        'Source RALPH-034N simulation blocks before validation.',
      ),
    );
    reasonCodes.add('source_blocked_before_validation');
    return {
      disposition: 'blocked_before_transition_planning',
      blockingFindings,
      reasonCodes: [...reasonCodes],
    };
  }
  if (sourceDisposition !== 'validation_requirements_identified') {
    blockingFindings.push(
      createFinding(
        'blocking',
        'unknown_source_approval_gate_disposition',
        'Unknown RALPH-034N approval gate disposition.',
  ...
  const blockedRequirements = evaluateBlockedRequirements(
    source.hypothetical_validation_requirements,
  );
...
export function buildRuntimeEvidenceTransitionSimulation(input = {}) {
  ...
  const dispositionEvaluation = evaluateTransitionDisposition(
    input,
    inputEvaluation,
    safetyEvaluation,
  );
  ...
  return {
    ...
    hypothetical_runtime_transitions: transitionsAllowedToList
      ? buildHypotheticalRuntimeTransitions(input)
      : [],
    hypothetical_evidence_transitions: transitionsAllowedToList
      ? buildHypotheticalEvidenceTransitions(input)
      : [],
    future_workflow_prerequisites: transitionsAllowedToList
      ? [
          'Future authorized validation execution must pass required checks.',
          'Future authorized validation evidence must be recorded.',
          'Future human review must accept the result.',
          'Future authorized review evidence must be recorded.',
          'Future authorized runtime transition writer must apply task/run transitions.',
        ]
      : [
...
export function formatRuntimeEvidenceTransitionSimulationPretty(simulation) {
  ...
  lines.push(
    'Execution: planning-only runtime/evidence transition simulation; no runtime state writes; no task history writes; no run history writes; no validation evidence writes; no review evidence writes; no validation commands; no review acceptance; no queued task execution; no prompt execution; no worker invocation; no adapter invocation; no provider/model invocation; no network activity; no report/run-log writing; no file writes; no staging; no commit; no push.',
  );
  lines.push('');
  lines.push('Hypothetical runtime transitions:');
  for (const entry of simulation.hypothetical_runtime_transitions)
    lines.push(
      `- ${entry.transition_id}: ${entry.from_status} -> ${entry.to_status}; mutation_performed=${entry.mutation_performed}; event_written=${entry.event_written}`,
    );
  ...
  for (const entry of simulation.hypothetical_evidence_transitions)
    lines.push(
      `- ${entry.transition_id}: ${entry.required_record_type}; evidence_written=${entry.evidence_written}`,
    );
