...
export function validatePostChangeReviewGateInput(input = {}) {
  ...
  if (!isPlainObject(input)) {
    blockingFindings.push(
      createFinding(
        'blocking',
        'invalid_input',
        'Input must be a single RALPH-034M post-change review-gate simulation object.',
      ),
    );
    return { valid: false, blocking_findings: blockingFindings };
  }

  if (input.phase !== SOURCE_POST_CHANGE_REVIEW_GATE_PHASE)
    blockingFindings.push(
      createFinding(
        'blocking',
        'invalid_source_phase',
        `Expected source phase ${SOURCE_POST_CHANGE_REVIEW_GATE_PHASE}.`,
  ...
  if (!isPlainObject(input.summary))
    blockingFindings.push(
      createFinding('blocking', 'missing_summary', 'Input is missing required summary object.'),
    );
  if (!isPlainObject(input.review_authorization))
    blockingFindings.push(
      createFinding(
        'blocking',
        'missing_review_authorization',
        'Input is missing required review_authorization object.',
      ),
    );
...
export function evaluateSourceSafetyInvariants(source = {}) {
  ...
  for (const field of SOURCE_AUTHORIZATION_FIELDS) {
    if (source[field] === true || source.review_authorization?.[field] === true)
      blockingFindings.push(
        createFinding(
          'blocking',
  ...
  if (source.not_review_evidence !== true || source.not_validation_evidence !== true)
    blockingFindings.push(
      createFinding(
        'blocking',
        'source_claims_evidence_authority',
        'Source must explicitly remain non-evidence.',
      ),
    );
...
export function buildHypotheticalValidationRequirements(categories = []) {
  const requirements = [
    requirement({
      requirementId: 'source_ralph_034m_valid',
      authority: 'RALPH-034M simulator contract',
      rationale:
        'The supplied post-change review-gate simulation must be structurally valid before any future validation approval consideration.',
    }),
    requirement({
      requirementId: 'safety_counters_zero_false',
      authority: '.governance/SAFETY.md',
      rationale:
        'All execution and mutation counters must remain zero/false before any future approval could be considered.',
    }),
    requirement({
      requirementId: 'human_review_required',
      authority: '.governance/REVIEW_POLICY.md',
      rationale: 'Human review remains mandatory; this simulator cannot accept review or approval.',
    }),
  ];
  ...
  for (const category of unique(categories)) {
    if (category === 'documentation-only')
      requirements.push(
        requirement({
          requirementId: 'verify_category_documentation_only',
          category,
          authority: 'VERIFY.md Category 1',
          requiredChecks: [
            'git --no-pager status --short',
            'git --no-pager diff --stat',
            'git --no-pager diff --name-only',
          ],
          rationale: 'Documentation-only changes require readback checks before completion claims.',
        }),
      );
    else if (category === 'governance-only')
      requirements.push(
        requirement({
          requirementId: 'verify_category_governance_only',
          category,
          authority: 'VERIFY.md Category 2',
          requiredChecks: [
            'git --no-pager status --short',
            'git --no-pager diff --stat',
            'git --no-pager diff --name-only',
          ],
          rationale: 'Governance-only changes require readback checks before completion claims.',
        }),
      );
    else if (category === 'test-only')
      requirements.push(
        requirement({
          requirementId: 'verify_category_test_only',
          category,
          authority: 'VERIFY.md Category 3',
          requiredChecks: [
            'task-relevant tests',
            'git --no-pager status --short',
            'git --no-pager diff --stat',
          ...
          rationale: 'Test-only changes require relevant tests and readback checks.',
        }),
      );
    else if (category === 'product-runtime-code')
      requirements.push(
        requirement({
          requirementId: 'verify_category_product_runtime_code',
          category,
          authority: 'VERIFY.md Category 4',
          requiredChecks: ['npm run verify'],
          rationale: 'Product/runtime code changes require full runtime verification.',
        }),
      );
    else if (category === 'edge-supabase')
      requirements.push(
        requirement({
          requirementId: 'verify_category_edge_supabase',
          category,
          authority: 'VERIFY.md Category 5',
          requiredChecks: [
          ...
          rationale:
            'Edge/Supabase changes require edge-specific verification before approval consideration.',
        }),
      );
    else if (category === 'dependency-change')
      requirements.push(
        requirement({
          requirementId: 'verify_category_dependency_change',
          category,
          authority: 'VERIFY.md Category 6 and .governance/SAFETY.md',
          requiredChecks: ['npm run verify', 'task-specific regression tests'],
          rationale:
            'Dependency changes require explicit authorization and full verification before approval consideration.',
        }),
      );
    else if (category === 'runtime-or-evidence-state')
      requirements.push(
        requirement({
          requirementId: 'blocked_category_runtime_or_evidence_state',
          category,
          authority: '.governance/SAFETY.md',
          requiredChecks: ['explicit future runtime/evidence mutation authorization'],
          rationale:
            'Runtime/evidence state changes are outside this planning-only simulator and fail closed.',
        }),
      );
    else if (category === 'protected_secret_or_env')
      requirements.push(
        requirement({
          requirementId: 'blocked_category_protected_secret_or_env',
          category,
          authority: '.governance/SAFETY.md',
          requiredChecks: ['human safety review', 'protected-file violation resolution'],
          rationale: 'Protected secret/environment changes must block approval consideration.',
        }),
      );
    else if (category === 'unknown-or-product-runtime-code')
      requirements.push(
        requirement({
          requirementId: 'blocked_category_unknown_or_product_runtime_code',
          category,
          authority: 'VERIFY.md Category Resolution Rule',
          requiredChecks: ['human classification review', 'strictest applicable verification'],
          rationale: 'Unknown change categories fail closed until classified by a human.',
        }),
      );
    else if (category === 'governance-script-only')
      requirements.push(
        requirement({
          requirementId: 'verify_category_governance_script_only',
          category,
          authority: 'VERIFY.md Category Resolution Rule',
          requiredChecks: [
            'node --check <changed script>',
            'task-relevant node tests',
            'git --no-pager status --short',
            'git --no-pager diff --stat',
            'git --no-pager diff --name-only',
          ],
          rationale:
            'Governance script changes require syntax checks, focused tests, and readback checks before approval consideration.',
        }),
      );
  }
...
function evaluateApprovalDisposition(source, inputEvaluation, safetyEvaluation) {
  ...
  if (blockingFindings.length > 0)
    return {
      disposition: 'no_future_approval_consideration',
      blockingFindings,
  ...
  if (sourceDisposition === 'invalid_input') {
    blockingFindings.push(
      createFinding('blocking', 'source_invalid_input', 'Source RALPH-034M simulation is invalid.'),
    );
    reasonCodes.add('invalid_input');
    return {
      disposition: 'blocked_before_validation',
      blockingFindings,
      reasonCodes: [...reasonCodes],
    };
  }
  if (sourceDisposition === 'would_reject_before_review') {
    blockingFindings.push(
      createFinding(
        'blocking',
        'source_would_reject_before_review',
        'Source RALPH-034M simulation rejects before review.',
      ),
    );
  ...
  for (const code of asArray(source?.reason_codes)) {
    if (BLOCKING_REASON_CODES.has(code)) {
      blockingFindings.push(
        createFinding('blocking', code, `Blocking RALPH-034M reason propagated: ${code}`),
      );
  ...
  for (const category of asArray(source?.summary?.validation_categories)) {
    if (BLOCKING_CATEGORIES.has(category)) {
      blockingFindings.push(
        createFinding(
          'blocking',
          `blocked_validation_category_${category}`,
          `Validation category fails closed before approval consideration: ${category}`,
          { category },
        ),
      );
  ...
  if (
    sourceDisposition === 'would_be_reviewable' ||
    sourceDisposition === 'would_require_human_review'
  ) {
    reasonCodes.add('hypothetical_validation_requirements_identified');
    return {
      disposition: 'validation_requirements_identified',
      blockingFindings,
      reasonCodes: [...reasonCodes],
    };
  }
  ...
  blockingFindings.push(
    createFinding(
      'blocking',
      'unknown_source_review_gate_disposition',
      'Unknown RALPH-034M review gate disposition.',
...
export function buildValidationApprovalGateSimulation(input = {}) {
  ...
  const requirements = buildHypotheticalValidationRequirements(
    asArray(input?.summary?.validation_categories),
  );
  const dispositionEvaluation = evaluateApprovalDisposition(
    input,
    inputEvaluation,
    safetyEvaluation,
  );
  ...
  return {
    ...
    approval_consideration: {
      future_approval_could_be_considered_after_requirements:
        dispositionEvaluation.disposition === 'validation_requirements_identified',
