...
function addFinding(state, code, details = {}, decision = ADMISSION_DECISIONS.REJECTED) {
  ...
  if (decision === ADMISSION_DECISIONS.REJECTED)
    state.admission_decision = ADMISSION_DECISIONS.REJECTED;
  else if (state.admission_decision === ADMISSION_DECISIONS.ADMISSIBLE)
...
function coerceMetadata(input, options) {
  ...
  const classifierOutput =
    typeof input.classification === 'string' && Array.isArray(input.reason_codes) ? input : null;
  ...
  return {
    metadata: {
      ...
      required_checks:
        taskMetadata.required_checks ?? input.required_verification ?? input.required_checks,
      evidence_requirements:
        taskMetadata.evidence_requirements ??
      ...
      non_authoritative_statement:
        taskMetadata.non_authoritative_statement ??
        input.non_authoritative_statement ??
        input.non_authorization_statement,
      ...
      approval_required_resolved:
        taskMetadata.approval_required_resolved ?? input.approval_required_resolved,
      protected_matches: taskMetadata.protected_matches ?? input.protected_matches,
      approval_required_matches:
        taskMetadata.approval_required_matches ?? input.approval_required_matches,
      ...
      existing_queue_entry_ids:
        taskMetadata.existing_queue_entry_ids ?? input.existing_queue_entry_ids,
...
function validateRequiredMetadata(metadata, state) {
  for (const field of REQUIRED_QUEUE_METADATA_FIELDS) {
    if (!(field in metadata) || metadata[field] === null || metadata[field] === undefined)
      addFinding(state, 'missing_queue_metadata', { field });
    else if (ARRAY_FIELDS.has(field) && !Array.isArray(metadata[field]))
      addFinding(state, 'malformed_queue_metadata', { field, expected: 'array' });
    else if (STRING_FIELDS.has(field) && typeof metadata[field] !== 'string')
      addFinding(state, 'malformed_queue_metadata', { field, expected: 'string' });
  }
  if (
    'review_requirement' in metadata &&
    typeof metadata.review_requirement !== 'boolean' &&
    typeof metadata.review_requirement !== 'string'
  ) {
    addFinding(state, 'malformed_queue_metadata', {
      field: 'review_requirement',
      expected: 'boolean_or_string',
    });
  }
}
...
function validateAdmissionConsistency(metadata, expectedDecision, state) {
  if (
    typeof metadata.admission_decision === 'string' &&
    metadata.admission_decision !== expectedDecision
  ) {
    addFinding(state, 'admission_decision_inconsistent', {
      declared: metadata.admission_decision,
...
function validateStringArray(metadata, field, state) {
  ...
  for (const value of values) {
    if (typeof value !== 'string' || value.trim() === '')
      addFinding(state, `${field}_malformed_entry`, { field });
  }
}
...
function validateFileArrays(metadata, state) {
  ...
  const missing = asArray(metadata.expected_changed_files)
    .map(normalizePath)
    .filter((file) => !allowed.has(file));
  if (missing.length > 0)
    addFinding(state, 'expected_changed_file_not_allowed', { files: uniqueSorted(missing) });
}

function validateRequiredDeclarations(metadata, state) {
  for (const field of ['required_checks', 'evidence_requirements', 'stop_conditions'])
    validateStringArray(metadata, field, state);
  if (String(metadata.commit_policy ?? '').toLowerCase() !== 'no_commit')
    addFinding(state, 'commit_policy_not_no_commit', { commit_policy: metadata.commit_policy });
  if (String(metadata.push_policy ?? '').toLowerCase() !== 'no_push')
    addFinding(state, 'push_policy_not_no_push', { push_policy: metadata.push_policy });
  if (!String(metadata.non_authoritative_statement ?? '').includes('non-authoritative'))
    addFinding(state, 'non_authoritative_statement_missing_marker');
}

function classifyProtectedMatches(metadata, state, options) {
  const protectedResult = options.protectedConfig
    ? parseProtectedConfig(options.protectedConfig)
    : readProtectedConfig(options.projectRoot || DEFAULT_PROJECT_ROOT);
  ...
  const files = uniqueSorted(
    [...asArray(metadata.allowed_files), ...asArray(metadata.expected_changed_files)].map(
      normalizePath,
    ),
  );
  const absolutePatterns = protectedResult.config.protected_patterns.absolute_protection.patterns;
  const conditionalPatterns =
    protectedResult.config.protected_patterns.conditional_protection.patterns;
  ...
  for (const file of files) {
    ...
    if (approval.length > 0 || conditional.some((entry) => entry.approval_required)) {
      approvalRequiredMatches.push({
        file,
        patterns: [...approval, ...conditional.map((entry) => entry.pattern)],
      });
    }
  }
  ...
  if (allProtected.length > 0)
    addFinding(state, 'protected_file_match_blocks_queue_admission', {
      files: uniqueSorted(allProtected.map((entry) => entry.file ?? entry)),
    });
  if (allApproval.length > 0 && metadata.approval_required_resolved !== true) {
    addFinding(
      state,
      'approval_required_unresolved_blocks_queue_admission',
      { files: uniqueSorted(allApproval.map((entry) => entry.file ?? entry)) },
      ADMISSION_DECISIONS.REQUIRES_REVIEW_BEFORE_QUEUE,
    );
  }
...
function validateGitSignals(metadata, state) {
  if (metadata.dirty_tree === true || String(metadata.git_status_short ?? '').trim() !== '')
    addFinding(state, 'dirty_tree_blocks_queue_admission');
  if (
    asArray(metadata.staged_files).length > 0 ||
    String(metadata.git_diff_cached_name_only ?? '').trim() !== ''
  )
    addFinding(state, 'staged_files_block_queue_admission', {
      staged_files: asArray(metadata.staged_files),
    });
}

function deterministicQueueEntryId(metadata) {
  if (typeof metadata.queue_entry_id === 'string' && metadata.queue_entry_id.trim())
    return metadata.queue_entry_id.trim();
  const taskId =
    typeof metadata.task_id === 'string' && metadata.task_id.trim()
      ? metadata.task_id.trim().toLowerCase()
      : 'missing-task-id';
  return `preview-${taskId}`;
}

function validateQueueCollision(metadata, queueEntryId, state) {
  if (metadata.queue_entry_collision === true)
    addFinding(state, 'queue_entry_collision_blocks_queue_admission', {
  ...
  for (const entry of asArray(metadata.existing_queue_entries)) {
    if (isPlainObject(entry) && typeof entry.queue_entry_id === 'string')
      existingIds.add(entry.queue_entry_id);
  }
  if (existingIds.has(queueEntryId))
    addFinding(state, 'queue_entry_collision_blocks_queue_admission', {
      queue_entry_id: queueEntryId,
    });
}
...
export function validateQueueAdmission(input, options = {}) {
  ...
  const expectedDecision =
    CLASSIFICATION_TO_ADMISSION[initialClassification] || ADMISSION_DECISIONS.REJECTED;
  ...
  return {
    ...
    admission_allowed:
      state.admission_decision === ADMISSION_DECISIONS.ADMISSIBLE && uniqueReasons.length === 0,
    executable: false,
    ...
    non_authorization_statement:
      'This validator is read-only advisory output only; it does not write queue entries, mutate runtime state, execute tasks, stage, commit, push, deploy, install dependencies, run formatters, run fixers, or access the network.',
  };
}
...
export function readAdmissionMetadataFile(inputPath, options = {}) {
  if (!isSafeRelativeMetadataFile(inputPath))
    throw new Error(
      'Unsafe metadata-file path: must be relative JSON without traversal or drive qualification',
    );
  ...
  if (!absolute.startsWith(rootWithSep))
    throw new Error('Unsafe metadata-file path resolved outside project root');
