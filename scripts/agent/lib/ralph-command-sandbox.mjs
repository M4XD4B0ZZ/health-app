...
const NON_AUTHORIZATION_STATEMENT =
  'RALPH-036B is a minimal read-only command sandbox. It authorizes only fixed command IDs mapped in this file, defaults to plan-only mode, never accepts shell command strings, and does not authorize arbitrary shell execution, mutation-capable commands, staging, commits, pushes, deploys, package mutation, formatting, fixing, network access, or runtime/evidence/governance/product mutation.';
...
export const RALPH_READONLY_COMMAND_ALLOWLIST = Object.freeze({
  git_status_short: Object.freeze({
    ...
    command_category: COMMAND_CATEGORIES.git_status_short,
    executable: 'git',
    args: ['--no-pager', 'status', '--short'],
    timeout_ms: DEFAULT_TIMEOUT_MS,
  }),
  git_diff_stat: Object.freeze({
    command_id: 'git_diff_stat',
    command_category: COMMAND_CATEGORIES.git_diff_stat,
    executable: 'git',
    args: ['--no-pager', 'diff', '--stat'],
    timeout_ms: DEFAULT_TIMEOUT_MS,
  }),
  git_diff_name_only: Object.freeze({
    command_id: 'git_diff_name_only',
    command_category: COMMAND_CATEGORIES.git_diff_name_only,
    executable: 'git',
    args: ['--no-pager', 'diff', '--name-only'],
    timeout_ms: DEFAULT_TIMEOUT_MS,
  }),
  node_check_self: Object.freeze({
    command_id: 'node_check_self',
    command_category: COMMAND_CATEGORIES.node_check_self,
    executable: 'node',
    args: ['--check', 'scripts/agent/lib/ralph-command-sandbox.mjs'],
    timeout_ms: DEFAULT_TIMEOUT_MS,
  }),
  node_check_cli: Object.freeze({
    command_id: 'node_check_cli',
    command_category: COMMAND_CATEGORIES.node_check_cli,
    executable: 'node',
    args: ['--check', 'scripts/agent/ralph-command-sandbox.mjs'],
    timeout_ms: DEFAULT_TIMEOUT_MS,
  }),
  node_test_self: Object.freeze({
    command_id: 'node_test_self',
    command_category: COMMAND_CATEGORIES.node_test_self,
    executable: 'node',
    args: ['--test', 'scripts/agent/__tests__/ralph-command-sandbox.test.mjs'],
    timeout_ms: 120_000,
  }),
});
...
const FORBIDDEN_GIT_MUTATIONS = new Set([
  'add',
  'commit',
  'push',
  'reset',
...
const FORBIDDEN_TEXT_PATTERNS = Object.freeze([
  ...
  [
    'env_printing_forbidden',
    /\b(printenv|env|set|Get-ChildItem\s+Env:|gci\s+env:|process\.env)\b/i,
  ],
  [
    'package_mutation_forbidden',
    /\b(npm|npx|pnpm|yarn)\b|\b(package\.json|package-lock\.json)\b|\binstall\b|\baudit\s+fix\b/i,
  ],
...
function commandText(spec) {
  return [spec?.executable, ...(Array.isArray(spec?.args) ? spec.args : [])].map(String).join(' ');
}

function appendBounded(current, chunk, limit) {
  if (byteLength(current) >= limit) return { value: current, truncated: true };
  const text = chunk.toString('utf8');
  if (byteLength(`${current}${text}`) <= limit)
...
export function validateCommandTextForRejection(value) {
  ...
  if (SHELL_WRAPPERS.has(executable))
    findings.push(
      finding('shell_wrapper_forbidden', `Shell wrapper is forbidden: ${firstToken}`, {
        value: text,
      }),
    );
  if (FORBIDDEN_EXECUTABLES.has(executable))
    findings.push(
      finding('forbidden_executable', `Executable is forbidden: ${firstToken}`, { value: text }),
    );
  for (const [code, pattern] of FORBIDDEN_TEXT_PATTERNS)
    if (pattern.test(text))
      findings.push(finding(code, `Forbidden command pattern detected: ${code}`, { value: text }));
  if (/\bgit\s+([A-Za-z-]+)/i.test(text)) {
    const subcommand = text.match(/\bgit\s+([A-Za-z-]+)/i)[1].toLowerCase();
    if (FORBIDDEN_GIT_MUTATIONS.has(subcommand))
      findings.push(
        finding('git_mutation_forbidden', `Git mutation command is forbidden: ${subcommand}`, {
          value: text,
          subcommand,
        }),
      );
...
export function validateAllowlistedCommandSpec(commandSpec) {
  ...
  if (!commandSpec || typeof commandSpec !== 'object')
    return {
      valid: false,
      blocking_findings: [finding('invalid_command_spec', 'Command spec must be an object.')],
    };
  if (!Object.hasOwn(RALPH_READONLY_COMMAND_ALLOWLIST, commandSpec.command_id))
    findings.push(
      finding(
        'command_not_in_ralph_036b_allowlist',
        'Only the fixed RALPH-036B allowlist is allowed.',
  ...
  if (SHELL_WRAPPERS.has(executable))
    findings.push(
      finding(
        ...
        `Shell wrapper executable is forbidden: ${commandSpec.executable}`,
      ),
    );
  if (FORBIDDEN_EXECUTABLES.has(executable))
    findings.push(
      finding('forbidden_executable', `Executable is forbidden: ${commandSpec.executable}`),
    );
  if (!Array.isArray(commandSpec.args) || commandSpec.args.some((arg) => typeof arg !== 'string'))
    findings.push(finding('invalid_args_shape', 'Command args must be fixed strings.'));
  ...
  if (
    executable === 'git' &&
    FORBIDDEN_GIT_MUTATIONS.has(String(commandSpec.args?.[1] || '').toLowerCase())
  )
    findings.push(
      finding('git_mutation_forbidden', 'Git mutation subcommand is forbidden.', {
...
export function planRalphReadonlyCommand(commandId = 'git_status_short', options = {}) {
  if (options.allowlist && options.testMode !== true)
    return baseResult({
      commandId,
      commandSpec: null,
      ...
      blockingFindings: [
        finding(
          'custom_allowlist_requires_test_mode',
          'Custom allowlists are test hooks only and require explicit testMode: true.',
        ),
      ],
    });
  ...
  if (!commandSpec)
    return baseResult({
      ...
      status: 'blocked',
      blockingFindings: [
        finding('unknown_command_id', `Unknown RALPH-036B command ID: ${commandId}`, {
  ...
  if (!validation.valid)
    return baseResult({
      ...
      allowed: false,
      status: 'blocked',
      blockingFindings: validation.blocking_findings,
    });
  const result = baseResult({
    ...
    allowed: true,
    status: 'planned',
    reasonCodes: ['plan_only_no_spawn'],
  });
...
export async function runRalphReadonlyCommand(commandId = 'git_status_short', options = {}) {
  ...
  if (options.allowlist && options.testMode !== true)
    return baseResult({
      commandId,
      commandSpec: null,
      executionMode: 'execute_readonly_command',
      allowed: false,
      status: 'blocked',
      blockingFindings: [
        finding(
  ...
  if (options.skipAllowlistValidation === true && options.testMode !== true)
    return baseResult({
      commandId,
      commandSpec: null,
      executionMode: 'execute_readonly_command',
      ...
      blockingFindings: [
        finding(
          'skip_allowlist_validation_requires_test_mode',
          'Skipping allowlist validation is a test hook only and requires explicit testMode: true.',
        ),
      ],
    });
  const allowlist = options.allowlist || RALPH_READONLY_COMMAND_ALLOWLIST;
  ...
  const validation =
    options.skipAllowlistValidation === true
      ? { valid: true, blocking_findings: [] }
      : validateAllowlistedCommandSpec(commandSpec);
  if (!validation.valid)
    return baseResult({
      commandId,
      commandSpec,
  ...
  return new Promise((resolve) => {
    ...
    const finish = (exitCode, signal, extraFindings = []) => {
      ...
      const result = baseResult({
        ...
        allowed: true,
        status,
        blockingFindings: extraFindings,
        timeoutMs,
        reasonCodes: [status],
      });
    ...
    const timer = setTimeout(() => {
      ...
      stopChild('SIGTERM');
      setTimeout(() => {
        if (!settled) stopChild('SIGKILL');
      }, TIMEOUT_KILL_GRACE_MS);
    }, timeoutMs);
    try {
      child = spawn(commandSpec.executable, commandSpec.args, {
        cwd,
        shell: false,
      ...
      finish(null, null, [
        finding('spawn_failed', 'Read-only command spawn failed.', { message: error.message }),
      ]);
      return;
    }
    ...
    child.on('error', (error) =>
      finish(null, null, [
        finding('spawn_error', 'Read-only command emitted spawn error.', {
