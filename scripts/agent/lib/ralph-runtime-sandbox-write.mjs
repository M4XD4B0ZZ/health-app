...
const NON_AUTHORIZATION_STATEMENT =
  'This RALPH-035B sandbox runtime-state writer is a supervised, bounded, non-authoritative create-only capability. It can create exactly one fixed sandbox JSON file only when --write-sandbox-state is explicitly supplied. It does not mutate canonical runtime, evidence, governance, product, package, validation, review, handoff, git, or deployment state.';
...
export function validateSandboxRuntimeStatePath(pathValue = SANDBOX_RUNTIME_STATE_PATH) {
  ...
  if (requestedPath.split('/').includes('..'))
    return {
      valid: false,
      path: requestedPath,
      code: 'path_traversal_refused',
      message: 'Path traversal is forbidden.',
    };
  if (requestedPath !== SANDBOX_RUNTIME_STATE_PATH)
    return {
      valid: false,
      path: requestedPath,
      code: 'not_allowlisted_sandbox_path',
      message: 'Only the fixed RALPH-035B sandbox runtime-state path is allowed.',
    };
  return {
    valid: true,
    path: requestedPath,
    code: 'allowlisted_sandbox_runtime_state_path',
    message: 'Allowed fixed sandbox runtime-state path.',
  };
}

export function validateSandboxRuntimeStatePayload(payload = SANDBOX_RUNTIME_STATE_PAYLOAD) {
  if (hasAuthorityLikePayload(payload))
    return {
      valid: false,
      code: 'authority_like_payload_refused',
      message: 'Authority-like runtime/evidence/governance payload fields are forbidden.',
    };
  if (!exactSandboxPayload(payload))
    return {
      valid: false,
      code: 'payload_schema_mismatch',
      message: 'Sandbox payload must match exactly { "simulated": true }.',
    };
  return {
    valid: true,
    code: 'exact_sandbox_payload',
    message: 'Sandbox payload matches exactly { "simulated": true }.',
  };
}
...
function assertDirectoryContainment(executionRoot, targetDirectory) {
  ...
  if (realTargetDirectory === realRoot || realTargetDirectory.startsWith(`${realRoot}${path.sep}`))
    return;
...
function readbackExactSandboxPayload(target) {
  ...
  if (!exactSandboxPayload(parsed))
    throw new Error('Sandbox state readback did not match exact schema.');
  return parsed;
}
...
export function executeSandboxRuntimeStateWrite(options = {}) {
  ...
  if (!pathValidation.valid)
    blockingFindings.push(
      finding('blocking', pathValidation.code, pathValidation.message, {
        path: pathValidation.path,
      }),
    );
  if (!payloadValidation.valid)
    blockingFindings.push(finding('blocking', payloadValidation.code, payloadValidation.message));
  ...
  if (blockingFindings.length === 0 && writeMode) {
    try {
      ...
      blockingFindings.push(
        finding('blocking', 'write_failed', 'Sandbox runtime-state write failed or was refused.', {
  ...
  return {
    ...
    verification_readbacks: [
      'node --check scripts/agent/lib/ralph-runtime-sandbox-write.mjs',
      'node --check scripts/agent/ralph-runtime-sandbox-write.mjs',
      'node --test scripts/agent/__tests__/ralph-runtime-sandbox-write.test.mjs',
    ...
    execution_plan: {
      ...
      canonical_runtime_state_mutations: 0,
    ...
    safety_summary: {
      supervised_sandbox_runtime_state_writer: true,
      dry_run_by_default: true,
      writes_require_explicit_write_sandbox_state_flag: true,
      fixed_allowlisted_path_only: true,
      create_only: true,
      ...
      alternative_output_paths_refused: true,
      authority_like_payloads_refused: true,
      exact_schema_validation: true,
      readback_validation_after_write: true,
      no_arbitrary_output_paths: true,
      no_command_execution_capability: true,
      no_network_activity: true,
      no_canonical_runtime_mutation: true,
...
export function formatSandboxRuntimeStateWritePretty(result) {
  ...
  lines.push(
    '',
    'Execution: supervised sandbox runtime-state create capability; dry-run by default; write requires --write-sandbox-state; fixed allowlisted path only; exact payload only; no canonical runtime/evidence/governance/product/package/handoff mutation; no command execution capability; no network; no staging; no commit; no push; no deploy.',
    '',
