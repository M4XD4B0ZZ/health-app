...
export const ACCEPTABLE_REVIEW_STATUSES = new Set([
  'accepted',
  'pending',
  'missing',
  'not_required',
...
export function validateHandoffSchema(handoff) {
  ...
  if (!SUPPORTED_HANDOFF_SCHEMA_VERSIONS.has(handoff.schema_version)) {
    critical.push(
      createFinding(
        ...
        `Unsupported handoff schema_version: ${handoff.schema_version || 'missing'}`,
        { expected: Array.from(SUPPORTED_HANDOFF_SCHEMA_VERSIONS) },
      ),
    );
  }
  ...
  if (!handoff.handoff_id || typeof handoff.handoff_id !== 'string') {
    critical.push(
      createFinding(
        'critical',
        'missing_handoff_id',
        'Handoff is missing required string field handoff_id',
      ),
    );
  }
  if (!handoff.timestamp || Number.isNaN(Date.parse(handoff.timestamp))) {
    critical.push(
      createFinding(
        ...
        'Handoff is missing a valid ISO timestamp',
      ),
    );
  }
  if (!handoff.task || typeof handoff.task !== 'object' || Array.isArray(handoff.task)) {
    critical.push(
      createFinding('critical', 'missing_task_object', 'Handoff is missing required task object'),
    );
  } else {
    if (!handoff.task.task_id || typeof handoff.task.task_id !== 'string') {
      critical.push(
        createFinding(
          'critical',
          'missing_task_id',
          'Handoff is missing required string field task.task_id',
        ),
      );
    }
    if (!handoff.task.status || typeof handoff.task.status !== 'string') {
      critical.push(
        createFinding(
          'critical',
          'missing_task_status',
          'Handoff is missing required string field task.status',
        ),
      );
    }
    if (!handoff.task.title) {
      warnings.push(
        createFinding(
          'warning',
          'missing_task_title',
          'Task title metadata is not present in the handoff',
        ),
      );
    }
    if (!handoff.task.run_id) {
      warnings.push(
        createFinding(
          'warning',
          'missing_task_run_id',
          'Runtime run_id metadata is not present in the handoff',
        ),
      );
    }
  }
  ...
  if (
    !handoff.validation ||
    typeof handoff.validation !== 'object' ||
    Array.isArray(handoff.validation)
  ) {
    critical.push(
      createFinding(
        'critical',
        'missing_validation_object',
        'Handoff is missing required validation object',
      ),
    );
  } else {
    if (!handoff.validation.status || typeof handoff.validation.status !== 'string') {
      critical.push(
        createFinding(
          ...
          'Handoff is missing required string field validation.status',
        ),
      );
    }
    if (
      handoff.validation.validation_id === null ||
      handoff.validation.validation_id === undefined
    ) {
      warnings.push(
        createFinding(
          'warning',
          'missing_validation_id',
          'Validation status exists but optional validation_id evidence is missing',
        ),
      );
      ...
      critical.push(
        createFinding(
          ...
          'validation.validation_id must be null or a string',
        ),
      );
    }
    if (typeof handoff.validation.summary !== 'string') {
      critical.push(
        createFinding(
          ...
          'Handoff is missing required string field validation.summary',
        ),
      );
    }
  }
  ...
  if (!handoff.review || typeof handoff.review !== 'object' || Array.isArray(handoff.review)) {
    critical.push(
      createFinding(
        'critical',
        'missing_review_object',
        'Handoff is missing required review object',
      ),
    );
  } else {
    if (!handoff.review.status || typeof handoff.review.status !== 'string') {
      critical.push(
        createFinding(
          ...
          'Handoff is missing required string field review.status',
        ),
      );
    }
    if (handoff.review.review_id === null || handoff.review.review_id === undefined) {
      warnings.push(
        createFinding(
          'warning',
          'review_pending_or_missing_id',
          'Review evidence is pending or optional review_id evidence is missing',
        ),
      );
    } else if (typeof handoff.review.review_id !== 'string') {
      critical.push(
        createFinding('critical', 'invalid_review_id', 'review.review_id must be null or a string'),
      );
    }
    if (typeof handoff.review.summary !== 'string') {
      critical.push(
        createFinding(
          ...
          'Handoff is missing required string field review.summary',
        ),
      );
    }
  }
  ...
  if (!handoff.changes || typeof handoff.changes !== 'object' || Array.isArray(handoff.changes)) {
    critical.push(
      createFinding(
        'critical',
        'missing_changes_object',
        'Handoff is missing required changes object',
      ),
    );
  } else {
    if (!Array.isArray(handoff.changes.files_changed)) {
      critical.push(
        createFinding(
          ...
          'changes.files_changed must be an array',
        ),
      );
    }
    if (!Array.isArray(handoff.changes.artifacts_created)) {
      critical.push(
        createFinding(
          'critical',
          'invalid_artifacts_created',
          'changes.artifacts_created must be an array',
        ),
      );
    }
  }
  ...
  if (!handoff.issues || typeof handoff.issues !== 'object' || Array.isArray(handoff.issues)) {
    critical.push(
      createFinding(
        'critical',
        'missing_issues_object',
        'Handoff is missing required issues object',
      ),
    );
  } else {
    if (!Array.isArray(handoff.issues.critical)) {
      critical.push(
        createFinding('critical', 'invalid_critical_issues', 'issues.critical must be an array'),
      );
    }
    if (!Array.isArray(handoff.issues.warnings)) {
      critical.push(
        createFinding('critical', 'invalid_warning_issues', 'issues.warnings must be an array'),
      );
    }
  }
  ...
  if (typeof handoff.recommended_next_task !== 'string') {
    warnings.push(
      createFinding(
        'warning',
        'missing_recommended_next_task',
        'recommended_next_task metadata is incomplete or missing',
      ),
    );
  }
...
export function evaluateReviewDecision(handoff) {
  ...
  for (const finding of asArray(handoff.issues?.critical)) {
    critical.push(
      createFinding('critical', 'handoff_critical_finding', String(finding), {
        source: 'handoff.issues.critical',
      }),
    );
  }
  for (const finding of asArray(handoff.issues?.warnings)) {
    warnings.push(
      createFinding('warning', 'handoff_warning', String(finding), {
        source: 'handoff.issues.warnings',
      }),
    );
  }
  ...
  if (handoff.validation?.status !== undefined && isFailedValidation(handoff.validation.status)) {
    critical.push(
      createFinding('critical', 'validation_failed', `Validation status is ${validationStatus}`),
    );
  } else if (
    handoff.validation?.status !== undefined &&
    !isPassingValidation(handoff.validation.status)
  ) {
    warnings.push(
      createFinding(
        'warning',
        'validation_not_passed',
        `Validation status is ${validationStatus || 'missing'}`,
      ),
    );
  }
  ...
  if (handoff.task?.status !== undefined && !isDoneStatus(handoff.task.status)) {
    warnings.push(
      createFinding('warning', 'task_not_done', `Task status is ${taskStatus || 'missing'}`),
    );
  }
    ...
    if (reviewStatus === 'rejected') {
      critical.push(
        createFinding(
          'critical',
          'review_rejected',
          'Canonical handoff references rejected review evidence',
        ),
      );
      ...
      warnings.push(
        createFinding(
          ...
          'Canonical handoff references review evidence requiring changes',
        ),
      );
      ...
      warnings.push(
        createFinding(
          'warning',
          'non_blocking_review_concern',
          `Review status is ${reviewStatus || 'missing'}`,
        ),
      );
    }
  }
  ...
  if (critical.length > 0) {
    ...
    warnings.length === 0 &&
    isPassingValidation(validationStatus) &&
    isDoneStatus(taskStatus) &&
    ACCEPTABLE_REVIEW_STATUSES.has(reviewStatus)
  ) {
...
export function decisionReason(reviewResult, critical, warnings) {
  if (reviewResult === 'rejected')
    return `Rejected because ${critical.length} critical finding(s) block review acceptance.`;
  if (reviewResult === 'needs_changes')
    return `Needs changes because ${warnings.length} warning(s) or non-blocking concern(s) require review attention.`;
...
export function validateReviewDecisionObject(decision, options = {}) {
  ...
  if (decision.schema_version !== SCHEMA_VERSION)
    throw new Error('Decision has invalid schema_version');
  if (!decision.review_id || typeof decision.review_id !== 'string')
    throw new Error('Decision missing review_id');
  if (!decision.timestamp || Number.isNaN(Date.parse(decision.timestamp)))
    throw new Error('Decision has invalid timestamp');
  if (!decision.task_id || typeof decision.task_id !== 'string')
    throw new Error('Decision missing task_id');
  if (!SUPPORTED_REVIEW_RESULTS.has(decision.review_result))
    throw new Error(`${invalidResultMessage}: ${decision.review_result}`);
  if (typeof decision.decision_reason !== 'string' || decision.decision_reason.trim() === '')
    throw new Error('Decision missing decision_reason');
  if (!Array.isArray(decision.blocking_findings))
    throw new Error('Decision blocking_findings must be an array');
  if (!Array.isArray(decision.warnings)) throw new Error('Decision warnings must be an array');
  if (typeof decision.human_review_required !== 'boolean')
    throw new Error('Decision human_review_required must be boolean');
}
...
export function buildNormalizedReviewDecision(handoff, options = {}) {
  ...
  const decision = {
    ...
    human_review_required:
      typeof handoff.human_review_required === 'boolean' ? handoff.human_review_required : true,
    source: {
