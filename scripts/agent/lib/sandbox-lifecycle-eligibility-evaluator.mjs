...
const EVIDENCE_MARKER_STATES = Object.freeze(['evidence_bundled', 'awaiting_human_review']);
...
function checkForbiddenAuthorityClaims(input, findings) {
  for (const claim of FORBIDDEN_AUTHORITY_CLAIMS) {
    if (
      ...
      findings.push(
        finding('critical', 'forbidden_authority_claim', 'Forbidden authority claim detected', {
          claim,
          value: input[claim],
        }),
      );
...
export function evaluateSandboxLifecycleEligibility(input, options = {}) {
  ...
  if (input.sandbox !== true) {
    findings.push(
      finding('critical', 'sandbox_marker_missing', 'Input must include sandbox: true', {
  ...
  if (input.non_authoritative !== true) {
    findings.push(
      finding(
        ...
        'Input must include non_authoritative: true',
        { non_authoritative: input.non_authoritative },
      ),
    );
  }
  ...
  if (!hasOwn(input, 'task_id') && !hasOwn(input, 'queue_entry_id')) {
    findings.push(
      finding('critical', 'missing_identifier', 'Input must include task_id or queue_entry_id'),
    );
  }

  if (!hasOwn(input, 'lifecycle') || !isPlainObject(input.lifecycle)) {
    findings.push(
      finding('critical', 'lifecycle_object_missing', 'Input must include a lifecycle object'),
    );
  } else if (!hasOwn(input.lifecycle, 'state')) {
    findings.push(
      finding('critical', 'lifecycle_state_missing', 'Lifecycle object must include state'),
    );
  }
  ...
  if (
    findings.filter(
      (f) => f.code === 'canonical_scope_violation' || f.code === 'forbidden_authority_claim',
    ).length === 0
  ) {
...
function buildResult(input, findings, options) {
  ...
  return {
    ...
    eligible_for_human_consideration:
      decision === ELIGIBILITY_DECISIONS.ELIGIBLE_FOR_HUMAN_CONSIDERATION,
