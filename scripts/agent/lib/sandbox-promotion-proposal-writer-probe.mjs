...
function hasRequiredFalseAuthorityClaims(value) {
  ...
  return REQUIRED_FALSE_AUTHORITY_FLAGS.every(
    ...
    if (stat.isSymbolicLink()) {
      findings.push(
        finding(
          ...
          'Symlink path components are refused for the fixed canonical-boundary target',
          { path: current },
        ),
      );
      continue;
...
export function readProposalInputFileSync(inputPath, options = {}) {
  if (!isSafeRelativeProposalInputFile(inputPath))
    throw new Error(`Unsafe proposal input file path rejected: ${inputPath}`);
  ...
  if (relative.startsWith('..') || path.isAbsolute(relative))
    throw new Error('Unsafe proposal input file resolved outside project root');
...
export function validatePromotionProposalInput(proposal) {
  ...
  if (!isPlainObject(proposal)) {
    findings.push(
      finding(
        'critical',
        'proposal_input_not_object',
        'Promotion proposal input must be an object',
      ),
    );
  ...
  if (proposal.generator !== SOURCE_GENERATOR_ID) {
    findings.push(
      finding(
        'critical',
        'source_generator_mismatch',
        'Only RALPH-044A promotion proposal generator output is accepted',
        { generator: proposal.generator ?? null },
      ),
    );
  }
  if (proposal.proposal_created !== true) {
    findings.push(
      finding(
        ...
        'Input proposal must have proposal_created === true',
        { proposal_created: proposal.proposal_created ?? null },
      ),
    );
  }
  if (proposal.writes_performed !== false) {
    findings.push(
      finding(
        'critical',
        'proposal_writes_performed_not_false',
        'Input proposal must have writes_performed === false',
        { writes_performed: proposal.writes_performed ?? null },
      ),
    );
  }
  if (proposal.stdout_only !== true) {
    findings.push(
      finding(
        'critical',
        'proposal_stdout_only_not_true',
        'Input proposal must have stdout_only === true',
        { stdout_only: proposal.stdout_only ?? null },
      ),
    );
  }
  if (!allAuthorityFlagsFalse(proposal.authority_flags)) {
    findings.push(
      finding(
        'critical',
        'proposal_authority_flags_not_all_false',
        'All proposal authority flags must be false',
      ),
    );
  }
  if (
    !hasRequiredFalseAuthorityClaims(proposal) ||
    !hasRequiredFalseAuthorityClaims(proposal.authority_flags)
  ) {
    findings.push(
      finding(
        'critical',
        'forbidden_authority_claim',
        'Forbidden authority claims must be absent or false',
      ),
    );
  }
  ...
  if (!source) {
    findings.push(
      finding('critical', 'source_summary_missing', 'Input proposal must include source_summary'),
    );
  } else {
    if (source.sandbox !== true)
      findings.push(
        finding(
          'critical',
          'source_sandbox_marker_missing',
          'Source summary must be sandbox-only',
          { sandbox: source.sandbox ?? null },
        ),
      );
    if (source.non_authoritative !== true)
      findings.push(
        finding(
          'critical',
          'source_non_authoritative_marker_missing',
          'Source summary must be non-authoritative',
          { non_authoritative: source.non_authoritative ?? null },
        ),
      );
  }
  ...
  if (normalized !== TARGET_PATH) {
    findings.push(
      finding(
        ...
        'Only the fixed RALPH-044B canonical-boundary promotion proposal target is authorized',
  ...
  if (fs.existsSync(targetValidation.absolute_target)) {
    result.findings.push(
      finding(
        ...
        'Create-only canonical-boundary promotion proposal probe refuses to overwrite an existing file',
        { target_path: TARGET_PATH },
      ),
    );
    return result;
  ...
  if (!readbackValid) {
    result.findings.push(
      finding(
        'critical',
        'readback_mismatch',
        'Readback content/hash/payload did not match the deterministic expected promotion proposal probe artifact',
      ),
    );
  }
