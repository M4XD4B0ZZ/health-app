...
export const NON_AUTHORITATIVE_LIFECYCLE_STATEMENT = 'This sandbox lifecycle probe is non-authoritative advisory metadata only.';
...
const STATEMENT_FORBIDDEN_TERMS = /\b(queue_execution|worker_execution|task_execution|runtime_authority|evidence_mutation|review_acceptance|validation_pass|task_completion|staging|commit|push|deploy|dependency_install|network|product_work|canonical_queue_admission|queued|executing|committed|pushed|deployed|approved|merged|released)\b/i;
...
function baseResult({ findings = [], currentState = null, nextState = null, sandbox = false, nonAuthoritative = false, statement = null } = {}) {
  return {
...
export function validateSandboxLifecycleState(input) {
  ...
  if (FORBIDDEN_STATE_SET.has(currentState)) {
    findings.push(finding('critical', 'forbidden_lifecycle_state', 'Forbidden lifecycle states are refused in sandbox lifecycle metadata', { state: currentState }));
  } else if (!SANDBOX_STATE_SET.has(currentState)) {
    findings.push(finding('critical', 'unknown_lifecycle_state', 'Unknown lifecycle states are refused', { state: currentState }));
  }

  return baseResult({ findings, currentState, sandbox: true, nonAuthoritative: true, statement: NON_AUTHORITATIVE_LIFECYCLE_STATEMENT });
}

export function validateSandboxLifecycleTransition(currentStateInput, nextStateInput, options = {}) {
  ...
  findings.push(...currentValidation.findings.map((entry) => ({ ...entry, details: { ...entry.details, role: 'current_state' } })));
  findings.push(...nextValidation.findings.map((entry) => ({ ...entry, details: { ...entry.details, role: 'next_state' } })));
  ...
  if (findings.length === 0) {
    ...
    if (currentState === 'blocked' && nextState === 'draft') {
      if (options.human_authorized_recovery === true) {
        ...
        findings.push(finding('critical', 'blocked_recovery_requires_human_authorization', 'blocked -> draft recovery is refused unless human_authorized_recovery is true', { current_state: currentState, next_state: nextState }));
      }
      ...
      const skipped = !linearNext.includes(nextState) && SANDBOX_STATE_SET.has(nextState) && nextState !== 'blocked' && nextIndex > currentIndex;
      findings.push(finding('critical', skipped ? 'skipped_lifecycle_transition' : 'invalid_lifecycle_transition', 'Lifecycle transition is not in the allowed sandbox transition table', { current_state: currentState, next_state: nextState, allowed_next_states: [...directNext] }));
    }
  }

  return baseResult({ findings, currentState, nextState, sandbox: true, nonAuthoritative: true, statement: NON_AUTHORITATIVE_LIFECYCLE_STATEMENT });
}

function collectForbiddenAuthorityClaims(metadata, findings) {
  for (const claim of FORBIDDEN_AUTHORITY_SET) {
    if (hasOwn(metadata, claim) && metadata[claim] !== false && metadata[claim] !== null && metadata[claim] !== undefined) {
      findings.push(finding('critical', 'forbidden_authority_claim', 'Forbidden authority claims are refused in sandbox lifecycle metadata', { claim, value: metadata[claim] }));
    }
  }
}
...
function validateNonAuthoritativeStatement(statement, findings) {
  if (typeof statement !== 'string' || statement.trim() === '') {
    findings.push(finding('critical', 'missing_non_authoritative_statement', 'Lifecycle metadata must include a non-authoritative statement'));
    return;
  }
  if (!statement.includes('non-authoritative')) {
    findings.push(finding('critical', 'non_authoritative_statement_missing_marker', 'Lifecycle statement must contain the non-authoritative marker'));
  }
  if (STATEMENT_FORBIDDEN_TERMS.test(statement)) {
    findings.push(finding('critical', 'non_authoritative_statement_contains_authority_terms', 'Lifecycle statement must not contain execution or authority terms', { statement }));
  }
}
...
export function validateSandboxLifecycleMetadata(metadataInput) {
  ...
  if (!isPlainObject(metadataInput)) {
    findings.push(finding('critical', 'metadata_not_object', 'Lifecycle metadata must be an object'));
  }
  if (metadata.sandbox !== true) {
    findings.push(finding('critical', 'sandbox_true_required', 'Lifecycle metadata must include sandbox: true', { sandbox: metadata.sandbox }));
  }
  if (metadata.non_authoritative !== true) {
    findings.push(finding('critical', 'non_authoritative_true_required', 'Lifecycle metadata must include non_authoritative: true', { non_authoritative: metadata.non_authoritative }));
  }
  if (!hasOwn(metadata, 'lifecycle') || !isPlainObject(metadata.lifecycle)) {
    findings.push(finding('critical', 'lifecycle_object_required', 'Lifecycle metadata must include a lifecycle object'));
  }
  if (!hasOwn(lifecycle, 'state')) {
    findings.push(finding('critical', 'lifecycle_state_required', 'Lifecycle metadata must include lifecycle.state'));
  } else {
    findings.push(...validateSandboxLifecycleState(currentState).findings);
  }
  if (hasOwn(lifecycle, 'next_state')) {
    findings.push(...validateSandboxLifecycleTransition(currentState, nextState, { human_authorized_recovery: metadata.human_authorized_recovery === true }).findings);
  }
