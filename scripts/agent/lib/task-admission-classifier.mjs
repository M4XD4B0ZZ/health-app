...
const VERIFY_STRICTNESS = Object.freeze({
  docs_only: 0,
  documentation: 0,
  focused: 1,
  agent_tooling: 1,
...
export function readTaskMetadataFile(inputPath, options = {}) {
  if (!isSafeRelativeTaskFile(inputPath))
    throw new Error(
      'Unsafe task-file path: must be relative JSON without traversal or drive qualification',
    );
  ...
  if (!absolute.startsWith(rootWithSep))
    throw new Error('Unsafe task-file path resolved outside project root');
...
function validateMetadata(task, state) {
  ...
  for (const field of REQUIRED_METADATA_FIELDS) {
    if (!(field in task))
      addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'missing_required_metadata', { field });
    else if (ARRAY_FIELDS.has(field) && !Array.isArray(task[field]))
      addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'ambiguous_metadata_type', {
        field,
        expected: 'array',
      });
    else if (STRING_FIELDS.has(field) && typeof task[field] !== 'string')
      addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'ambiguous_metadata_type', {
        field,
        expected: 'string',
      });
  }
  if (typeof task.requires_human_review !== 'boolean')
    addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'ambiguous_metadata_type', {
...
function classifyProtectedMatches(files, protectedConfig, state) {
  ...
  for (const file of files) {
    ...
    const conditional_matches = conditionalPatterns.filter((entry) =>
      matchesPattern(file, entry.pattern),
    );
    ...
    if (
      ...
      approvalRequiredMatches.push({
        ...
        patterns: [
          ...
          ...conditional_matches.map((entry) => entry.pattern),
        ],
      });
    }
  }
  if (protectedMatches.length > 0)
    addSignal(state, CLASSIFICATIONS.FORBIDDEN, 'protected_file_match', {
      files: protectedMatches.map((entry) => entry.file),
    });
  if (approvalRequiredMatches.length > 0)
    addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'approval_required_file_match', {
      files: approvalRequiredMatches.map((entry) => entry.file),
    });
...
function classifyActions(task, protectedConfig, state) {
  const actions = uniqueSorted(
    [...(task.forbidden_actions || []), task.commit_policy, task.goal, task.title].map((value) =>
  ...
  for (const action of actions) {
    if (neverAllowed.includes(action) || forbiddenWords.some((word) => action.includes(word)))
      addSignal(state, CLASSIFICATIONS.FORBIDDEN, 'forbidden_action_match', { action });
    if (approvalActions.includes(action))
      addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'approval_required_action_match', {
        action,
      });
  }
  if (String(task.push_policy || '').toLowerCase() !== 'no_push')
    addSignal(state, CLASSIFICATIONS.FORBIDDEN, 'push_policy_not_no_push', {
...
function classifyTaskShape(task, files, state) {
  ...
  if (task.requires_human_review === true)
    addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'metadata_requires_human_review');
  if (taskType.includes('agent'))
    addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'agent_tooling_task_type', {
      task_type: task.task_type,
    });
  if (taskType.includes('product') || files.some((file) => file.startsWith('src/')))
    addSignal(state, CLASSIFICATIONS.HUMAN_ONLY, 'product_code_signal');
  if (
    files.some((file) => file === 'package.json' || file === 'package-lock.json') &&
    !taskType.includes('dependency')
  )
    addSignal(
      state,
      CLASSIFICATIONS.REVIEW_REQUIRED,
      'package_change_without_explicit_dependency_task',
    );
  if (files.some((file) => file.includes('secret') || file === '.env' || file.startsWith('.env.')))
    addSignal(state, CLASSIFICATIONS.FORBIDDEN, 'secret_or_env_path_signal');
  if (risk === 'high' || risk === 'critical')
    addSignal(state, CLASSIFICATIONS.HUMAN_ONLY, 'high_risk_signal', {
      risk_level: task.risk_level,
    });
  if (risk === 'medium')
    addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'medium_risk_signal', {
      risk_level: task.risk_level,
    });
}

function classifyVerification(task, state) {
  const categories = Array.isArray(task.verification_category)
    ? task.verification_category
    : [task.verification_category];
  let strictest = 'docs_only';
  for (const raw of categories) {
    const key = String(raw ?? 'unknown').toLowerCase();
    if (
      (VERIFY_STRICTNESS[key] ?? VERIFY_STRICTNESS.unknown) >
      (VERIFY_STRICTNESS[strictest] ?? VERIFY_STRICTNESS.unknown)
    )
      strictest = key;
  }
  const score = VERIFY_STRICTNESS[strictest] ?? VERIFY_STRICTNESS.unknown;
  if (score === 1)
    addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'verification_requires_review', {
      strictest,
    });
  if (score === 2)
    addSignal(state, CLASSIFICATIONS.HUMAN_ONLY, 'verification_requires_human', { strictest });
  if (score === 3)
    addSignal(state, CLASSIFICATIONS.FORBIDDEN, 'verification_forbidden_category', { strictest });
...
function classifyThresholds(task, state) {
  ...
  const fileCount = Number(
    task.expected_file_count ?? task.diff_file_count ?? task.expected_changed_files?.length ?? 0,
  );
  ...
  if (fileCount > maxReviewFiles || lineCount > maxReviewLines)
    addSignal(state, CLASSIFICATIONS.HUMAN_ONLY, 'diff_threshold_human_only', {
      file_count: fileCount,
      line_count: lineCount,
    });
  else if (fileCount > maxSafeFiles || lineCount > maxSafeLines)
    addSignal(state, CLASSIFICATIONS.REVIEW_REQUIRED, 'diff_threshold_review_required', {
      file_count: fileCount,
      line_count: lineCount,
    });
}
...
export function classifyTaskAdmission(task, options = {}) {
  const protectedConfigResult = options.protectedConfig
    ? parseProtectedConfig(options.protectedConfig)
    : loadProtectedConfig(options.projectRoot || DEFAULT_PROJECT_ROOT);
  const state = {
    classification: CLASSIFICATIONS.SAFE_AUTONOMOUS,
    reason_codes: [],
    matched_signals: [],
  };
  if (!protectedConfigResult.ok)
    addSignal(state, CLASSIFICATIONS.HUMAN_ONLY, protectedConfigResult.reason);
  validateMetadata(task, state);
  const files = task && typeof task === 'object' ? allCandidateFiles(task) : [];
  const protectedScope = protectedConfigResult.ok
    ? classifyProtectedMatches(files, protectedConfigResult.config, state)
    : { protectedMatches: [], approvalRequiredMatches: [] };
  ...
  const verify =
    task && typeof task === 'object'
      ? classifyVerification(task, state)
      : { categories: [], strictest: null };
  return {
    ...
    review_requirement:
      ...
      state.classification === CLASSIFICATIONS.REVIEW_REQUIRED ||
      state.classification === CLASSIFICATIONS.HUMAN_ONLY,
    stop_conditions: Array.isArray(task?.stop_conditions) ? task.stop_conditions : [],
    non_authorization_statement:
      'This classifier is read-only advisory output only; it does not authorize execution, mutation, staging, commits, pushes, deploys, network access, or queue admission by itself.',
  };
}
