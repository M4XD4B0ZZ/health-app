...
export function parseArgs(argv) {
  ...
  for (const arg of args) {
    if (arg === '--pretty') options.pretty = true;
      ...
      throw new Error(
        `Execution, worker, adapter, provider, model, prompt, diff, validation, review, approval, evidence, runtime, output, write, commit, stage, or push flag is forbidden for approval readiness simulator: ${arg}`,
      );
...
export function readHumanApprovalCheckpointSimulation(simulationPath) {
  return JSON.parse(fs.readFileSync(resolveSimulationPath(simulationPath), 'utf8'));
}
...
async function main() {
  ...
  try {
    const simulation = buildApprovalReadinessSimulation(
      readHumanApprovalCheckpointSimulation(options.simulationPath),
    );
    ...
    console.error(
      JSON.stringify(
        buildApprovalReadinessSimulation({
          ...
          error: {
            code: 'human_approval_checkpoint_simulation_read_or_parse_failed',
