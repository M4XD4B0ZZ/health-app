...
import { executeDocsOnlyOperation, formatDocsOnlyExecutorPretty } from './lib/overnight-docs-only-executor.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const projectRoot = path.resolve(__dirname, '../..');
const EXIT_CODES = Object.freeze({ OK: 0, INVALID_INPUT: 1, REFUSED: 2 });
const REJECTED_FLAGS = new Set(['--worker', '--adapter', '--provider', '--model', '--prompt', '--validate', '--review', '--approve', '--stage', '--commit', '--push', '--output', '--write-runtime', '--write-evidence', '--write-report', '--write-run-log']);
...
export function parseArgs(argv) {
  ...
  for (const arg of args) {
    ...
    else if (REJECTED_FLAGS.has(arg)) throw new Error(`Execution, worker, adapter, provider, model, prompt, validation, review, approval, runtime, evidence, report, run-log, output, stage, commit, or push flag is forbidden for docs-only executor: ${arg}`);
...
async function main() {
  ...
  if (options.help || !options.inputPath) { console.error(usage()); process.exit(options.help ? EXIT_CODES.OK : EXIT_CODES.INVALID_INPUT); }
  try {
    const result = executeDocsOnlyOperation(readDocsOnlyInput(options.inputPath), { writeDocsOnly: options.writeDocsOnly, executionRoot: projectRoot });
    ...
    const result = executeDocsOnlyOperation({ error: { code: 'docs_only_input_read_or_parse_failed', message: error.message, input_path: options.inputPath } }, { writeDocsOnly: false, executionRoot: projectRoot });
