...
export function parseArgs(argv) {
  ...
  for (const arg of args) {
    if (arg === '--pretty') options.pretty = true;
      ...
      throw new Error(
        `Execution, worker, adapter, provider, model, prompt, validation, review, approval, write, output, stage, commit, or push flag is forbidden for execution capability gate simulator: ${arg}`,
      );
...
export function readApprovalReadinessSimulation(simulationPath) {
...
async function main() {
  ...
  try {
    const simulation = buildExecutionCapabilityGateSimulation(
      readApprovalReadinessSimulation(options.simulationPath),
    );
    ...
    console.error(
      JSON.stringify(
        buildExecutionCapabilityGateSimulation({
          ...
          error: {
            code: 'approval_readiness_simulation_read_or_parse_failed',
