...
export function parseArgs(argv) {
  ...
  for (const arg of args) {
    ...
    else if (REJECTED_FLAGS.has(arg)) throw new Error(`Execution, worker, adapter, provider, model, prompt, diff, validation, review, approval, evidence, runtime, write, commit, stage, or push flag is forbidden for runtime/evidence transition simulator: ${arg}`);
...
async function main() {
  ...
  try {
    ...
    const failure = buildRuntimeEvidenceTransitionSimulation({ phase: null, mode: null, reason_codes: ['invalid_input'], error: { code: 'validation_approval_gate_simulation_read_or_parse_failed', message: error.message, simulation_path: options.simulationPath } });
