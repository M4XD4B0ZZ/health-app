...
import { executeSandboxRuntimeStateWrite, formatSandboxRuntimeStateWritePretty, SANDBOX_RUNTIME_STATE_PATH } from './lib/ralph-runtime-sandbox-write.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const projectRoot = path.resolve(__dirname, '../..');
const EXIT_CODES = Object.freeze({ OK: 0, INVALID_INPUT: 1, REFUSED: 2 });
const REJECTED_FLAGS = new Set(['--worker', '--adapter', '--provider', '--model', '--prompt', '--validate', '--review', '--approve', '--stage', '--commit', '--push', '--deploy', '--output', '--path', '--payload', '--append', '--truncate', '--overwrite', '--write-runtime', '--write-evidence', '--write-governance', '--write-product', '--write-package', '--write-handoff', '--write-tasks', '--write-runs', '--write-validation', '--write-review']);
...
export function parseArgs(argv) {
  ...
  for (const arg of args) {
    ...
    else if (REJECTED_FLAGS.has(arg)) throw new Error(`Execution, worker, adapter, provider, model, prompt, validation, review, approval, output, path, payload, append, truncate, overwrite, canonical state, evidence, governance, product, package, handoff, stage, commit, push, or deploy flag is forbidden for sandbox runtime-state writer: ${arg}`);
...
async function main() {
  ...
  const result = executeSandboxRuntimeStateWrite({ writeSandboxState: options.writeSandboxState, executionRoot: projectRoot });
