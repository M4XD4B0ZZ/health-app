...
async function testQuery(funcName, query, expectedStatus) {
  ...
  if (status === 401) {
    console.error(
      `❌ FAILED on ${funcName}: Received 401 Invalid JWT. Ensure the correct project is linked and verify_jwt=false is deployed.`,
    );
    process.exit(1);
...
async function run() {
  console.log(
    `Verifying Edge Functions against REMOTE Supabase at Domain: ${new URL(url).hostname}\n`,
  );
  try {
