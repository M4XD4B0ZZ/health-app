...
async function verifyTableExists(tableName) {
  ...
  if (status === 404) {
    console.error(
      `❌ FAILED: Table '${tableName}' does not exist on the remote database (404 Not Found).`,
    );
...
async function run() {
  console.log(
    `Verifying Database Schema against REMOTE Supabase at Domain: ${new URL(url).hostname}\n`,
  );
  try {
