import {
  ComputeDailyProgressUseCase,
  GoalsNotFoundError,
} from '../application/usecases/ComputeDailyProgressUseCase';
...
describe('ComputeDailyProgressUseCase', () => {
  ...
  describe('error handling', () => {
    it('should throw GoalsNotFoundError when no goals are set', async () => {
      ...
      await expect(computeProgressUseCase.execute({ consumed })).rejects.toThrow(
        GoalsNotFoundError,
      );

      await expect(computeProgressUseCase.execute({ consumed })).rejects.toThrow(
        'Effective goals not found. Please set goals first.',
      );
    });
  });
