...
describe("Dish recognition", () => {
  test("exact known dish: Lasagne", () => {
    ...
    const interpretation = buildInputInterpretation({ raw: input, items: [{ name: input, quantity: null, unit: null, rawText: input }] }, [], { level: "high", score: 1, reason: "test" })
  ...
  test("non-dish multi-item input: 2 Eier und Toast", () => {
    ...
    const interpretation = buildInputInterpretation({ raw: input, items: [
      { name: "2 Eier", quantity: null, unit: null, rawText: "2 Eier" },
      { name: "Toast", quantity: null, unit: null, rawText: "Toast" }
