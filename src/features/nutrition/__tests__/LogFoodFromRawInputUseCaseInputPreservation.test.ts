...
const mockResolver = {
  resolve: jest.fn().mockImplementation(async (query: any, options: any) => {
    ...
    console.log(
      `[${traceId}] MOCK_RESOLVER_RECEIVED raw="${raw}" normalized="${normalized}" locale="${locale}"`,
    );

    // Simuliere erfolgreiche Auflösung für deutsche Inputs
    const isGermanInput = ['ei', 'eier', 'quark'].some(
      (term) => normalized.includes(term) || raw.includes(term),
    );
