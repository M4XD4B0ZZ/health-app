...
const mockResolver = {
  resolve: jest.fn().mockImplementation(async (query: any, options: any) => {
    ...
    console.log(`[${traceId}] PROOF_RESOLVER_CALL raw="${raw}" normalized="${normalized}" locale="${locale}"`);
    ...
    if (result) {
      console.log(`[${traceId}] PROOF_RESOLVER_SUCCESS normalized="${normalized}" candidates=${result.candidates.length} bestScore=${result.best?.score}`);
      return result;
    }
...
describe('P0-002 Proof Test - Fünf Kerninputs', () => {
  ...
  const testKernInput = async (rawInput: string, expectedParsedName: string) => {
    ...
    try {
      ...
      console.log(`PROOF_SUMMARY rawInput="${rawInput}" success=${proofData.success} macros=${JSON.stringify(proofData.macros)} persistSuccess=${proofData.persistSuccess}`);
